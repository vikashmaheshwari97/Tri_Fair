#!/usr/bin/env bash
# Submit Tri-Fair-v5 versus the unchanged full-development NSGA-II baseline.
...
export TF_OPTIMIZERS="${TF_OPTIMIZERS:-Tri-Fair-v5,NSGAII-PO-Fair}"
...
export MANIFEST_DIR="${MANIFEST_DIR:-data/splits_v5}"
...
export TF_RESULTS_NAMESPACE="${TF_RESULTS_NAMESPACE:-tri_fair_v5_qwen_5m}"
...
for optimizer in "${optimizers[@]}"; do
  tf_contains "$optimizer" Tri-Fair-v5 NSGAII-PO-Fair \
    || tf_die "Unsupported v5 optimizer '$optimizer'"
done
...
printf 'Tri-Fair-v5 submission plan\n'
printf '  models:       %s\n' "$TF_MODELS"
printf '  datasets:     %s\n' "$TF_DATASETS"
printf '  optimizers:   %s\n' "$TF_OPTIMIZERS"
printf '  seeds:        %s\n' "$TF_SEEDS"
...
if tf_is_true "$DRY_RUN"; then
  ...
  printf ' %q' "${SBATCH_ARGS[@]}" jobs/tri_fair_v5_main.sbatch
  printf '\n'
  exit 0
fi

job_id="$(sbatch "${SBATCH_ARGS[@]}" jobs/tri_fair_v5_main.sbatch)"
...
tf_log "Submitted Tri-Fair-v5 array job $job_id"
