#!/usr/bin/env bash
# Submit Tri-Fair-v3 versus the matched full-development NSGA-II baseline.
...
for optimizer in "${optimizers[@]}"; do
  tf_contains "$optimizer" Tri-Fair-v3 NSGAII-PO-Fair \
    || tf_die "Unsupported v3 optimizer '$optimizer'"
done
...
# Fail before submission when a requested resume grid is incomplete.
...
printf 'Tri-Fair-v3 submission plan\n'
printf '  models:       %s\n' "$TF_MODELS"
printf '  datasets:     %s\n' "$TF_DATASETS"
printf '  optimizers:   %s\n' "$TF_OPTIMIZERS"
printf '  seeds:        %s\n' "$TF_SEEDS"
...
if tf_is_true "$DRY_RUN"; then
  ...
  printf ' %q' "${SBATCH_ARGS[@]}" jobs/tri_fair_v3_main.sbatch
  printf '\n'
  exit 0
fi

job_id="$(sbatch "${SBATCH_ARGS[@]}" jobs/tri_fair_v3_main.sbatch)"
...
tf_log "Submitted Tri-Fair-v3 array job $job_id"
