...
SBATCH_ARGS=(--parsable "--array=$array_spec" "--job-name=tfv7-${BUDGET}")
...
printf 'Tri-Fair-v7 matched submission\n'
printf '  models: %s\n  datasets: %s\n  optimizers: %s\n' \
  "$TF_MODELS" "$TF_DATASETS" "$TF_OPTIMIZERS"
printf '  seeds: %s\n  budget: %s\n  namespace: %s\n' \
  "$TF_SEEDS" "$BUDGET" "$TF_RESULTS_NAMESPACE"
printf '  array: %s (%d tasks)\n' "$array_spec" "$total"

if tf_is_true "$DRY_RUN"; then
  printf 'DRY RUN: sbatch'
  printf ' %q' "${SBATCH_ARGS[@]}" jobs/tri_fair_v7_main.sbatch
  printf '\n'
  exit 0
fi

job_id="$(sbatch "${SBATCH_ARGS[@]}" jobs/tri_fair_v7_main.sbatch)"
tf_log "Submitted Tri-Fair-v7 array job $job_id"
