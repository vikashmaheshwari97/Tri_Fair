"""Evaluate the shared raw Initial Instructions pool for the v6 study."""
...
from src.config.v6_profiles import (
    V6_STUDY_VERSION,
    apply_v6_dataset_profile,
)
from src.helpers.llm_creation import create_llm
from src.helpers.task_creation import (
    create_dev_tasks,
    create_test_task,
)
...
def _attach_result(
    ...
    *,
    ...
    output_tokens = np.asarray(
        result.agg_output_tokens, dtype=float
    )
    cost = (
    ...
    output[f"{prefix}_fairness"] = np.asarray(
        result.fairness_loss, dtype=float
    )
...
def run(args: argparse.Namespace) -> Path:
    if args.n_init_prompts <= 0 or args.max_output_tokens <= 0:
        raise ValueError(
            "n-init-prompts and max-output-tokens must be positive"
        )
    ...
    dataset_config = apply_v6_dataset_profile(
        ALL_DATASETS[args.dataset]
    )
    if args.n_init_prompts > len(
    ...
    prompts = [
        Prompt(instruction=value, few_shots=[])
    ...
    output["selection_policy"] = (
        "shared_initial_pool_dev_selected_during_analysis"
    )
    output["study_profile"] = V6_STUDY_VERSION
