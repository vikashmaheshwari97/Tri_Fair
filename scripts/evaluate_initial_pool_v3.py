"""Evaluate the sampled raw initial instruction pool on v3 dev and holdout data.

This follows the MO-CAPO reporting convention: the initial Pareto set is chosen
from development objectives and its corresponding holdout values are reported as
an external baseline.  The evaluation is separate from the optimizer's 5M token
budget and must be labelled "Initial Instructions" rather than an optimizer run.
"""
...
def run(args: argparse.Namespace) -> Path:
    ...
    dataset_config = apply_v3_dataset_profile(ALL_DATASETS[args.dataset])
    ...
    output["selection_policy"] = "initial_pool_dev_selected_during_analysis"
