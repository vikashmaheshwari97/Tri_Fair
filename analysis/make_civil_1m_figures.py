"""MO-CAPO-style curated figures for Civil Comments / Qwen-3-30B / Tri-Fair.
...
Terminology:
  ...
  - For Civil Comments, unfairness = 1 - worst-group accuracy.
...
def output_dir() -> Path:
    out = Path("analysis/output/curated_figures_civil_mocapo_style")
...
def check_clean_counts(run_metrics: pd.DataFrame) -> None:
    ...
    if FINAL_BUDGET not in observed_budgets:
        raise RuntimeError(
            f"Expected budget_checkpoint={FINAL_BUDGET}, "
    ...
    for optimizer in OPTIMIZER_ORDER:
        ...
        if n != 3:
            raise RuntimeError(
                ...
                "Check Civil Comments evaluation outputs and rebuild analysis."
            )

    if 250_000 not in observed_budgets:
        warnings.warn(
            "No 250k Civil checkpoint found in run_metrics.csv. "
            "Figures will still be created for available checkpoints."
        )
...
def plot_stepwise_nR2_proxy(step_proxy: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.savefig(outdir / "civil_comments_qwen3_nR2_trajectory_stepwise.pdf", bbox_inches="tight")
    fig.savefig(outdir / "civil_comments_qwen3_nR2_trajectory_stepwise.png", bbox_inches="tight")
    plt.close(fig)
...
def plot_hv_gap_trajectory(
    ...
    checkpoints = sorted(int(v) for v in run_metrics["budget_checkpoint"].unique())
    xticks = [v / 1_000_000.0 for v in checkpoints]
    labels = ["250k" if v == 250_000 else "1M" if v == 1_000_000 else f"{v/1_000_000:g}M" for v in checkpoints]
    ax.set_xticks(xticks)
    ax.set_xticklabels(labels)

    axes[0].legend(frameon=False, loc="best")
    fig.suptitle("Civil Comments — Qwen-3-30B: Trajectory diagnostics", y=1.03)

    fig.savefig(outdir / "civil_comments_qwen3_hv_gap_trajectory_stepwise.pdf", bbox_inches="tight")
    fig.savefig(outdir / "civil_comments_qwen3_hv_gap_trajectory_stepwise.png", bbox_inches="tight")
    plt.close(fig)
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Civil Comments — Qwen-3-30B at 1M")
...
def output_cost_share(frame: pd.DataFrame) -> np.ndarray:
    if not {"test_input_tokens", "test_output_tokens"}.issubset(frame.columns):
...
def plot_trifair_fewshot_scatter(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Civil Comments — Qwen-3-30B at 1M")
        ...
        rows.append(
            {
                ...
                "Balanced Worst-Group Accuracy ↑": 1.0 - data["balanced_test_fairness"].mean(),
                "Seeds": len(data),
            }
        )

    table = pd.DataFrame(rows)
    table.to_csv(outdir / "civil_comments_qwen3_1m_summary_table.csv", index=False)
    (outdir / "civil_comments_qwen3_1m_summary_table.md").write_text(
...
def write_readme(outdir: Path) -> None:
    readme = """# Civil Comments / Qwen-3-30B 1M curated figures

Files generated by `analysis.make_civil_1m_figures`.

## Important note on nR2 trajectories

`civil_comments_qwen3_nR2_trajectory_stepwise.*` is a development-side stepwise nR2 proxy
...
## Civil Comments fairness

For Civil Comments:
- Test Accuracy is the user-facing quality metric.
- Test Unfairness is `1 - worst_group_accuracy`.
- Lower unfairness is better.
- Higher worst-group accuracy is better.

## Figures

- `civil_comments_qwen3_nR2_trajectory_stepwise.*`
  Stepwise development nR2 proxy, mean ± std across seeds.

- `civil_comments_qwen3_hv_gap_trajectory_stepwise.*`
  Stepwise development hypervolume plus checkpoint-level holdout approximation gap.

- `civil_comments_qwen3_1m_attainment_accuracy_cost.*`
  Empirical-attainment-style Test Accuracy vs Avg. Cost at the 1M checkpoint.

- `civil_comments_qwen3_1m_attainment_accuracy_unfairness.*`
  Empirical-attainment-style Test Accuracy vs Test Unfairness at the 1M checkpoint.

- `civil_comments_qwen3_1m_trifair_fewshot_outputshare.*`
  Tri-Fair final incumbent/evaluated candidates. Labels show few-shot count;
  color shows weighted output-token cost share.

- `civil_comments_qwen3_1m_trifair_fewshot_unfairness.*`
...
def main() -> None:
    ...
    step_proxy.to_csv(
        outdir / "civil_comments_qwen3_stepwise_dev_nR2_proxy.csv",
        index=False,
    )
    ...
    plot_empirical_attainment(
        ...
        filename="civil_comments_qwen3_1m_attainment_accuracy_cost",
    )
    ...
    plot_empirical_attainment(
        ...
        filename="civil_comments_qwen3_1m_attainment_accuracy_unfairness",
    )
    ...
    plot_trifair_fewshot_scatter(
        ...
        filename="civil_comments_qwen3_1m_trifair_fewshot_outputshare",
        cmap="viridis",
    )
    ...
    plot_trifair_fewshot_scatter(
        ...
        filename="civil_comments_qwen3_1m_trifair_fewshot_unfairness",
        cmap="viridis",
    )
    ...
    print("\nCurated MO-CAPO-style Civil Comments figures written to:")
