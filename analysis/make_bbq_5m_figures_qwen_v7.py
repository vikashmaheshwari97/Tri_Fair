"""Create publication-style BBQ 5M figures for the final Tri-Fair-v7 study.

This is the Qwen3-30B / BBQ adaptation of
...
Safeguards
----------
1. Exactly the intended six BBQ optimisation runs are accepted.
...
4. Main BBQ figures require ``test_fairness_ready`` and diagnostic
   ``coverage_valid=True``. Invalid candidates remain only in the reference
   coverage diagnostic and audit tables.
...
Default Rocket usage::
    ...
    python -m analysis.make_bbq_5m_figures_qwen_v7 --rebuild-analysis --strict
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/bbq_qwen3_30b_v7_final5m",
        help="Dedicated Qwen3-30B BBQ analysis-table directory.",
    )
    parser.add_argument(
        "--figure-dir",
        default=None,
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_bbq_qwen3_30b_v7_final5m.json",
        help="Dedicated deterministic Qwen3-30B v7 BBQ bounds file; incompatible legacy bounds are rejected.",
    )
    parser.add_argument(
        "--cost-upper-bound",
        type=float,
        default=FIXED_COST_UPPER_BOUND,
        help="Fixed cost-objective upper bound used by the BBQ configuration.",
    )
    parser.add_argument(
        "--overwrite-bounds",
        action="store_true",
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Raise on unreadable artifacts or missing BBQ fairness diagnostics.",
    )
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended final Qwen3-30B / BBQ / v7 rows."""
    out = frame.copy()
    if "run_dir" in out:
...
def attach_bbq_validity(frame: pd.DataFrame) -> pd.DataFrame:
    """Parse BBQ diagnostic JSON and attach explicit validity columns."""
    out = frame.copy()
...
def publication_valid_evaluations(
    ...
    *,
    ...
    if strict:
        ...
        if missing:
            raise ValueError(f"Evaluation table lacks BBQ validity fields: {missing}")
...
def ensure_qwen_bounds(
    ...
    *,
    ...
    """Create or validate deterministic BBQ normalization bounds."""
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="bbq_qwen3_30b_v7_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
    print(f"Wrote deterministic BBQ bounds: {path.resolve()}")
    return store
...
def _load_shared_union_final_evaluations(
    ...
    *,
    ...
    if errors:
        ...
        warnings.warn(message)
    if not frames:
        raise RuntimeError("No final shared-union BBQ evaluations were loaded")
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild final BBQ analysis from shared-union holdout and six run histories."""
    del budgets
    ...
    print(f"\nRebuilding Qwen3-30B BBQ v7 5M tables from: {results_root.resolve()}")
...
def read_inputs(
    ...
    if missing:
        raise FileNotFoundError(
            "Missing required BBQ analysis outputs:\n"
            + "\n".join(missing)
...
def infer_cost_bounds(
    ...
    *,
    ...
    """Return the fixed BBQ cost bound; data-dependent rescaling is forbidden."""
...
def plot_nr2_development_and_holdout(
    ...
    *,
    ...
    fig.suptitle(
        "BBQ — Qwen3-30B: Development Dynamics and Final Holdout",
        fontsize=12,
    )
    save_figure(
        fig,
        outdir,
        "bbq_qwen3_30b_v7_5m_final_nr2_development_and_holdout",
    )
...
def plot_stepwise_coverage_validity(
    ...
    *,
    ...
    ax.set_title("BBQ — Development Coverage Validity")
    ax.set_xlabel("Cumulative Downstream Tokens [×10⁶]")
    ...
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_stepwise_coverage_validity")
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("BBQ — Qwen3-30B")
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("BBQ — Qwen3-30B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_exact_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    axes[0].legend(frameon=False, loc="best")
    fig.suptitle("BBQ — Qwen3-30B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("BBQ — Qwen3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("BBQ — Qwen3-30B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Accuracy ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("BBQ — Qwen3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on BBQ — Qwen3-30B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Accuracy")
    ax.grid(True, alpha=0.25)
...
def plot_coverage_validity_diagnostics(final_raw: pd.DataFrame, outdir: Path) -> None:
    ...
    summary.to_csv(
        outdir / "bbq_qwen3_30b_v7_5m_final_coverage_validity_by_seed.csv", index=False
    )
    ...
    fig.suptitle("BBQ — Qwen3-30B at 5M")
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_coverage_validity_diagnostics")
...
def plot_high_accuracy_operating_points(
    ...
    *,
    ...
    points.to_csv(
        outdir / "bbq_qwen3_30b_v7_5m_final_high_accuracy_operating_points.csv",
        index=False,
    )
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 3.9), constrained_layout=True)
    specifications = [
    ...
    axes[0].legend(frameon=False)
    fig.suptitle(
        f"BBQ — Lowest Unfairness with Test Accuracy ≥ {threshold:.2f}"
    )
    save_figure(fig, outdir, "bbq_qwen3_30b_v7_5m_final_high_accuracy_operating_points")
    return points
...
def write_summary_tables(
    ...
    table.to_csv(outdir / "bbq_qwen3_30b_v7_5m_final_summary_table.csv", index=False)
    (outdir / "bbq_qwen3_30b_v7_5m_final_summary_table.md").write_text(
        table.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    coverage.to_csv(
        outdir / "bbq_qwen3_30b_v7_5m_final_coverage_summary.csv", index=False
    )
    (outdir / "bbq_qwen3_30b_v7_5m_final_coverage_summary.md").write_text(
        coverage.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    print("\nBBQ 5M method summary")
...
def write_readme(
    ...
    *,
    ...
    content = f"""# BBQ / Qwen3-30B / Tri-Fair-v7 / 5M publication figures

Generated by `python -m analysis.make_bbq_5m_figures_qwen_v7`.
...
- `bbq_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`: Panel A is the development-only anytime proxy; Panel B shows the exact 5M holdout nR2 for seeds 72, 73, and 74 with method mean ± SD in the legend.
...
- `bbq_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`
- `bbq_qwen3_30b_v7_5m_final_stepwise_coverage_validity.*`
- `bbq_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap.*`
- `bbq_qwen3_30b_v7_5m_final_exact_nr2_hv_gap.*`
- `bbq_qwen3_30b_v7_5m_final_balanced_exact_metrics.*`
- `bbq_qwen3_30b_v7_5m_final_attainment_accuracy_cost.*`
- `bbq_qwen3_30b_v7_5m_final_attainment_accuracy_unfairness.*`
- `bbq_qwen3_30b_v7_5m_final_pareto_cost_unfairness.*`
- `bbq_qwen3_30b_v7_5m_final_test_pareto_3d.*`
- `bbq_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap.*`
- `bbq_qwen3_30b_v7_5m_final_coverage_validity_diagnostics.*`
- `bbq_qwen3_30b_v7_5m_final_high_accuracy_operating_points.*`
- `bbq_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare.*`
- `bbq_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    selected.to_csv(
        figure_dir / "bbq_qwen3_30b_v7_5m_final_selected_runs.csv", index=False
    )
    checkpoint_metrics.to_csv(
        figure_dir / "bbq_qwen3_30b_v7_5m_final_checkpoint_run_metrics.csv", index=False
    )
    final_evaluations.to_parquet(
        figure_dir / "bbq_qwen3_30b_v7_5m_final_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir / "bbq_qwen3_30b_v7_5m_final_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
        figure_dir / "bbq_qwen3_30b_v7_5m_final_stepwise_dev_nr2_proxy.csv", index=False
    )
    ...
    plot_empirical_attainment(
        ...
        filename="bbq_qwen3_30b_v7_5m_final_attainment_accuracy_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="bbq_qwen3_30b_v7_5m_final_attainment_accuracy_unfairness",
    )
    ...
    plot_trifair_diagnostic(
        ...
        filename="bbq_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="bbq_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness",
    )
