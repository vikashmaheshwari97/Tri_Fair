"""Build Tri-Fair v5 all-step holdout and Initial Instructions metric tables."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="analysis/output/v5")
...
def _bounds(dataset: str) -> Bounds:
    config = apply_v5_dataset_profile(ALL_DATASETS[dataset])
    ...
    return Bounds(
        ...
        source="tri_fair_v5_fixed_semantic_bounds",
    )


def _numeric_max(frame: pd.DataFrame, column: str, *, default: int | float) -> float:
    if column not in frame:
        ...
        rows.append(
            {
                ...
                "chosen_step": int(_numeric_max(group, "chosen_step", default=0)),
                "actual_budget_tokens": int(
                    _numeric_max(group, "actual_budget_tokens", default=int(budget))
                ),
                **metrics,
            }
        )
...
def main() -> None:
    ...
    if metrics.empty:
        raise RuntimeError("No valid v5 holdout or initial metrics were computed")
    ...
    metrics.to_csv(output_dir / "v5_checkpoint_run_metrics.csv", index=False)
    summary.to_csv(output_dir / "v5_checkpoint_summary.csv", index=False)
    compact.to_csv(output_dir / "v5_nr2_hv_gap_table.csv", index=False)
    markdown_path = output_dir / "v5_nr2_hv_gap_table.md"
    try:
