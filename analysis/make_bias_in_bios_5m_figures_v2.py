"""Create publication-style Bias-in-Bios 5M v2 figures for Tri-Fair and NSGA-II-PO-Fair.

This script is specific to the upgraded Bias-in-Bios study stored under
...
Key safeguards
--------------
1. Only the six intended Qwen-3-30B / Bias-in-Bios / 5M runs are accepted:
   two optimizers × seeds 42, 43, and 44.
2. Exact holdout metrics are computed only from the selected 5M run for each
   optimizer/seed pair; incomplete or duplicate checkpoint rows cannot be
   counted as independent observations.
3. Main multi-objective metrics require both ``dev_fairness_ready`` and
   ``test_fairness_ready``. Macro-recall shortfalls are not silently
   discarded because Bias-in-Bios v2 already folds them into the optimized
   unfairness objective through ``utility_penalty``.
4. Objective normalization is deterministic and matches the v2 Bias-in-Bios objective
   bounds: quality loss in [0, 1], weighted mean-token cost in [0, 120], and
...
Default usage on Rocket::
    ...
    python -m analysis.make_bias_in_bios_5m_figures_v2 --rebuild-analysis --strict

Generated outputs include development anytime trajectories, exact 5M
multi-objective comparisons, readiness-valid Pareto and attainment figures,
macro-recall and profession-conditional gender TPR-gap diagnostics, high-quality operating-point comparisons,
...
# The current study has exact holdout evaluation at 5M.  Intermediate values
# are useful only for development-side anytime trajectories.
DEFAULT_BUDGETS = (1_000_000, 2_000_000, 3_000_000, 4_000_000, 5_000_000)
EXPECTED_SEEDS = (42, 43, 44)
N_PREFERENCES = 1_000
PREFERENCE_SEED = 2026
REFERENCE_POINT = (1.1, 1.1, 1.1)
FIXED_COST_UPPER_BOUND = 120.0
HIGH_QUALITY_FRACTION = 0.90
...
UNFAIRNESS_AXIS_LABEL = "Statistical Gender TPR-Gap Unfairness ↓"
...
class CostBounds:
    ...
    def normalize_minimize(self, raw: np.ndarray) -> np.ndarray:
        """Normalize minimize-all objectives [1-macro-F1, cost, unfairness]."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/bias_in_bios_5m_v2",
        help="Dedicated Bias-in-Bios v2 analysis-table directory.",
    )
    parser.add_argument(
        "--figure-dir",
        default=None,
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_bias_in_bios_v2.json",
        help="Dedicated deterministic Bias-in-Bios v2 bounds file.",
    )
    parser.add_argument(
        "--cost-upper-bound",
        type=float,
        default=FIXED_COST_UPPER_BOUND,
        help="Fixed Bias-in-Bios cost-objective upper bound.",
    )
    parser.add_argument(
        "--overwrite-bounds",
        action="store_true",
        help="Replace an incompatible dedicated Bias-in-Bios v2 bounds file.",
    )
    parser.add_argument(
        "--budgets",
    ...
    parser.add_argument(
        ...
        type=float,
        default=HIGH_QUALITY_FRACTION,
        help=(
            "For each run, choose the lowest-unfairness point whose macro-F1 is "
            "at least this fraction of that run's best macro-F1."
        ),
    )
    parser.add_argument(
        "--rebuild-analysis",
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended Bias-in-Bios/Qwen runs and remove archived copies."""
    out = frame.copy()
    if "run_dir" in out:
...
def attach_bios_diagnostics(frame: pd.DataFrame) -> pd.DataFrame:
    """Parse Bias-in-Bios v2 diagnostics and attach audit-friendly columns.

    The native unfairness objective is the larger of the profession-conditional
    gender TPR-gap RMS and the normalized macro-recall shortfall below 0.20.
    Utility-penalized candidates are therefore retained rather than removed.
    """
    ...
    for split in ("dev", "test"):
        ...
        out[f"{split}_statistically_ready"] = (
            out[ready_column].fillna(False).astype(bool)
            if ready_column in out
            else pd.Series(False, index=out.index)
        )

        required = out[f"{split}_required_macro_recall"].fillna(0.20)
        observed = out[f"{split}_macro_recall"]
        penalty = out[f"{split}_utility_penalty"].fillna(0.0)
        out[f"{split}_utility_valid"] = observed.ge(required)
        out[f"{split}_utility_penalized"] = penalty.gt(1e-12)

    out["publication_valid"] = (
        out["dev_statistically_ready"] & out["test_statistically_ready"]
    )
    out["test_ready_only"] = out["test_statistically_ready"]
    return out
...
def publication_valid_evaluations(
    ...
    *,
    ...
    """Keep rows with statistically ready development and holdout objectives.

    Bias-in-Bios v2 folds macro-recall shortfall directly into ``test_fairness``.
    Utility-penalized rows remain legitimate candidates because excluding them
    would change the optimized objective after the fact.
    """

    enriched = attach_bios_diagnostics(frame)
    if strict:
        missing = [
            column
            for column in (
        ...
        if missing:
            raise ValueError(
                f"Evaluation table lacks Bias-in-Bios v2 fields: {missing}"
            )

        for split in ("dev", "test"):
            metric_column = f"{split}_fairness_diagnostics_json"
            metrics = enriched[metric_column].map(
                lambda value: _json_object(value).get("metric")
            )
            unexpected = sorted(
                {
                    str(value)
                    for value in metrics.dropna().unique()
                    if str(value) != "bios_tpr_gap_statistical"
                }
            )
            if unexpected:
                raise ValueError(
                    f"Unexpected {split} fairness metrics: {unexpected}"
                )

    valid = enriched[enriched["publication_valid"]].copy()
    if valid.empty:
        raise RuntimeError(
            "No development/test-ready Bias-in-Bios rows remain after "
            "statistical-readiness filtering"
        )
...
def ensure_v2_bounds(
    ...
    *,
    ...
    """Create or validate deterministic Bias-in-Bios v2 objective bounds."""
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="bias_in_bios_v2_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
    print(f"Wrote deterministic Bias-in-Bios v2 bounds: {path.resolve()}")
    return store
...
def write_analysis_manifest(
    ...
    *,
    ...
    payload = {
        ...
        "validity_rule": "dev_fairness_ready and test_fairness_ready",
        "utility_rule": (
            "macro recall below the configured threshold is retained "
            "and penalized inside the native unfairness objective"
        ),
        "cost_definition": (
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild exact v2 tables using readiness-valid Bias-in-Bios rows."""
    ...
    print(f"\nRebuilding upgraded Bias-in-Bios 5M tables from: {results_root.resolve()}")
    ...
    raw_evaluations = attach_bios_diagnostics(raw_evaluations)
    ...
    print(
        ...
        f"({len(valid_evaluations)}/{len(raw_evaluations)} rows development/test ready)"
    )
...
def read_inputs(
    ...
    if missing:
        raise FileNotFoundError(
            "Missing required Bias-in-Bios v2 analysis outputs:\n"
            + "\n".join(missing)
    ...
    raw_evaluations = attach_bios_diagnostics(
...
def stepwise_development_r2_proxy(
    ...
    *,
    ...
    for keys, group in step_results.groupby(
        group_cols,
        sort=True,
        dropna=False,
    ):
        ...
        valid_mask = finite_mask & ready
        if not valid_mask.any():
            continue

        raw = np.column_stack(
            [
                1.0 - quality[valid_mask],
                cost[valid_mask],
                fairness[valid_mask],
            ]
        )
        ...
        macro_recall = np.full(len(group), np.nan, dtype=float)
        utility_penalty = np.full(len(group), np.nan, dtype=float)
        valid_professions = np.full(len(group), np.nan, dtype=float)
        if "fairness_diagnostics_json" in group:
            diagnostics = group["fairness_diagnostics_json"].map(_json_object)
            macro_recall = pd.to_numeric(
                diagnostics.map(
                    lambda value: value.get("macro_recall", np.nan)
                ),
                errors="coerce",
            ).to_numpy(dtype=float)
            utility_penalty = pd.to_numeric(
                diagnostics.map(
                    lambda value: value.get("utility_penalty", np.nan)
                ),
                errors="coerce",
            ).to_numpy(dtype=float)
            valid_professions = pd.to_numeric(
                diagnostics.map(
                    lambda value: value.get("valid_professions", np.nan)
                ),
                errors="coerce",
            ).to_numpy(dtype=float)

        ready_fraction = (
            float(np.mean(ready[finite_mask]))
            if finite_mask.any()
            else float("nan")
        )

        valid_penalties = utility_penalty[valid_mask]
        utility_valid_fraction = (
            float(np.mean(valid_penalties <= 1e-12))
            if np.isfinite(valid_penalties).any()
            else float("nan")
        )

        valid_recalls = macro_recall[valid_mask]
        mean_macro_recall = (
            float(np.nanmean(valid_recalls))
            if np.isfinite(valid_recalls).any()
            else float("nan")
        )

        valid_profession_counts = valid_professions[valid_mask]
        mean_valid_professions = (
            float(np.nanmean(valid_profession_counts))
            if np.isfinite(valid_profession_counts).any()
            else float("nan")
        )
        ...
        if "total_tokens_downstream" in group:
            tokens = int(
                np.nanmax(
                    finite_numeric(group["total_tokens_downstream"])
                )
            )
        else:
            input_tokens = (
                np.nanmax(
                    finite_numeric(group["input_tokens_downstream"])
                )
            ...
            output_tokens = (
                np.nanmax(
                    finite_numeric(group["output_tokens_downstream"])
                )
        ...
        rows.append(
            {
                **meta,
                ...
                "dev_noisy_r2_proxy": chebyshev_r2(
                    normalized[front_mask],
                    weights,
                ),
                "dev_proxy_front_size": int(front_mask.sum()),
                "dev_fairness_ready_fraction": ready_fraction,
                "dev_utility_valid_fraction": utility_valid_fraction,
                "dev_mean_macro_recall": mean_macro_recall,
                "dev_mean_valid_professions": mean_valid_professions,
            }
        )

    if not rows:
        raise RuntimeError(
            "Could not compute Bias-in-Bios stepwise development nR2 proxy"
        )
...
def plot_stepwise_nr2_proxy(
    ...
    *,
    ...
    ax.set_title("Bias-in-Bios v2 — Qwen-3-30B")
    ...
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_stepwise_dev_nr2_proxy")



def plot_stepwise_readiness_and_utility(
    step_proxy: pd.DataFrame,
    outdir: Path,
    *,
    max_budget: int,
    ...
    specifications = [
        (
            ...
            "Statistically Ready Candidate Fraction",
            "Ready Fraction",
            (0.0, 1.05),
        ),
        (
            "dev_utility_valid_fraction",
            "Macro-Recall-Valid Candidate Fraction",
            "Recall-Valid Fraction",
            (0.0, 1.05),
        ),
        (
            "dev_mean_macro_recall",
            "Mean Development Macro Recall",
            "Macro Recall",
            (0.0, 1.0),
        ),
    ]

    for ax, (column, title, ylabel, limits) in zip(
        axes,
        specifications,
    ):
        stats = step_band_statistics(
        ...
        for optimizer in OPTIMIZER_ORDER:
            ...
            ax.fill_between(
                x,
                np.clip(lower, limits[0], limits[1]),
                np.clip(upper, limits[0], limits[1]),
                step="post",
                color=COLORS[optimizer],
                alpha=0.16,
            )
    ...
    axes[2].axhline(
        ...
        linewidth=1,
        linestyle="--",
        label="Required macro recall",
    )
    axes[0].legend(frameon=False)
    axes[2].legend(frameon=False, fontsize=8)

    fig.suptitle(
        "Bias-in-Bios v2 — Development Readiness and Utility"
    )
    save_figure(
        fig,
        outdir,
        "bias_in_bios_qwen3_5m_v2_stepwise_readiness_utility",
    )
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("Bias-in-Bios v2 — Qwen-3-30B")
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Bias-in-Bios v2 — Qwen-3-30B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_exact_nr2_hv_gap")


def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 3.9), constrained_layout=True)
    specifications = [
        ("balanced_test_quality", "Balanced Test Macro-F1 ↑", "Macro-F1"),
    ...
    if handles:
        axes[0].legend(frameon=False, loc="best")
    fig.suptitle("Bias-in-Bios v2 — Qwen-3-30B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Bias-in-Bios v2 — Qwen-3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Bias-in-Bios v2 — 5M Cost vs Test Unfairness")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Bias-in-Bios v2 — Qwen-3-30B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Macro-F1 ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Bias-in-Bios v2 — Qwen-3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "bias_in_bios_qwen3_5m_v2_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Bias-in-Bios v2 — Qwen-3-30B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Macro-F1")
    ax.grid(True, alpha=0.25)
...
def utility_validity_summary(final_raw: pd.DataFrame) -> pd.DataFrame:
    data = attach_bios_diagnostics(final_raw)
    rows: list[dict[str, object]] = []
    ...
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            ...
            total = int(len(group))
            ready = (
                int(group["publication_valid"].sum())
                if total
                else 0
            )
            utility_valid = (
                int(group["test_utility_valid"].sum())
                if total
                else 0
            )
            penalized = (
                int(group["test_utility_penalized"].sum())
                if total
                else 0
            )

            signed = pd.to_numeric(
                group["test_mean_signed_tpr_gap"],
                errors="coerce",
            )
            ...
            rows.append(
                {
                    ...
                    "development_and_test_ready_candidates": ready,
                    "macro_recall_valid_candidates": utility_valid,
                    ...
                    "macro_recall_valid_fraction": (
                        float(utility_valid / total)
                        if total
                        else float("nan")
                    ),
                    "mean_macro_recall": pd.to_numeric(
                        group["test_macro_recall"],
                        errors="coerce",
                    ).mean(),
                    "mean_rms_tpr_gap": pd.to_numeric(
                        group["test_rms_tpr_gap"],
                        errors="coerce",
                    ).mean(),
                    "mean_max_abs_tpr_gap": pd.to_numeric(
                        group["test_max_abs_tpr_gap"],
                        errors="coerce",
                    ).mean(),
                    "mean_abs_signed_tpr_gap": signed.abs().mean(),
                    "mean_utility_penalty": pd.to_numeric(
                        group["test_utility_penalty"],
                        errors="coerce",
                    ).mean(),
                    "mean_valid_professions": pd.to_numeric(
                        group["test_valid_professions"],
                        errors="coerce",
                    ).mean(),
                    "maximum_ci_width": pd.to_numeric(
                        group["test_max_ci_width"],
                        errors="coerce",
                    ).max(),
                }
            )
...
def plot_fairness_validity_diagnostics(
    final_raw: pd.DataFrame,
    outdir: Path,
) -> None:
    data = attach_bios_diagnostics(final_raw)
    summary = utility_validity_summary(data)
    summary.to_csv(
        outdir / "bias_in_bios_qwen3_5m_v2_validity_by_seed.csv",
        index=False,
    )
    ...
    for index, optimizer in enumerate(OPTIMIZER_ORDER):
        ...
        axes[0, 0].bar(
            positions + offset,
            method["macro_recall_valid_fraction"],
            width=width,
            color=COLORS[optimizer],
            alpha=0.82,
            label=DISPLAY_NAME[optimizer],
    ...
    axes[0, 0].set_ylabel("Candidate Fraction")
    axes[0, 0].set_ylim(0.0, 1.05)
    axes[0, 0].set_title("Macro Recall ≥ 0.20")
    axes[0, 0].grid(
        True,
        axis="y",
        alpha=0.25,
    ...
    # B: native disparity and macro-recall gate.
    penalized = data[
        data["test_utility_penalized"]
    ]
    unpenalized = data[
        ~data["test_utility_penalized"]
    ]
    axes[0, 1].scatter(
        penalized["test_rms_tpr_gap"],
        penalized["test_macro_recall"],
        marker="x",
        color="0.5",
        alpha=0.80,
        s=42,
        label="Utility-penalized",
    )
    for optimizer in OPTIMIZER_ORDER:
        group = unpenalized[
            unpenalized["optimizer"] == optimizer
        ]
        axes[0, 1].scatter(
            group["test_rms_tpr_gap"],
            group["test_macro_recall"],
    ...
    axes[0, 1].set_xlabel(
        "Profession-Conditional Gender TPR-Gap RMS ↓"
    )
    axes[0, 1].set_ylabel("Macro Recall ↑")
    axes[0, 1].set_title(
        "Native Disparity and Utility Gate"
    )
    ...
    for index, optimizer in enumerate(OPTIMIZER_ORDER):
        ...
        jitter = (
            np.full(len(method), index, dtype=float)
            + np.linspace(-0.08, 0.08, len(method))
        )
        axes[1, 0].scatter(
            jitter,
            method["test_max_ci_width"],
            marker=MARKERS[optimizer],
            color=COLORS[optimizer],
            alpha=0.58,
            s=28,
    ...
    axes[1, 0].set_ylabel("Maximum Wilson CI Width ↓")
    axes[1, 0].set_title(
        "Statistical Precision Across Professions"
    )
    axes[1, 0].grid(
        True,
        axis="y",
    ...
    components = [
        ("test_rms_tpr_gap", "RMS TPR gap"),
        ("test_max_abs_tpr_gap", "Max |TPR gap|"),
        ("test_mean_signed_tpr_gap", "|Mean signed gap|"),
    ]
    x = np.arange(
        len(components),
        dtype=float,
    ...
    for index, optimizer in enumerate(OPTIMIZER_ORDER):
        ...
        for column, _ in components:
            ...
            if column == "test_mean_signed_tpr_gap":
                values = values.abs()
            means.append(float(values.mean()))
    ...
    axes[1, 1].set_ylabel("TPR Disparity ↓")
    axes[1, 1].set_title(
        "Gender TPR-Gap Diagnostics"
    )
    axes[1, 1].grid(
        True,
        axis="y",
    ...
    fig.suptitle(
        "Bias-in-Bios v2 — Qwen-3-30B at 5M"
    )
    save_figure(
        fig,
        outdir,
        "bias_in_bios_qwen3_5m_v2_fairness_diagnostics",
    )
...
def high_quality_operating_points(
    ...
    *,
    ...
    """Select lowest-unfairness points retaining a fraction of run-best macro-F1."""

    if not 0.0 < fraction <= 1.0:
        raise ValueError(
            "--high-quality-fraction must be in the interval (0, 1]"
        )

    data = attach_bios_diagnostics(final)
    rows: list[dict[str, object]] = []
    ...
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            ...
            if eligible.empty:
                raise RuntimeError(
                    f"No high-quality point for "
                    f"{optimizer}/seed{seed}; "
                    f"floor={quality_floor:.6f}"
                )
            ...
            rows.append(
                {
                    ...
                    "method": DISPLAY_NAME[optimizer],
                    "seed": seed,
                    "quality_fraction": float(fraction),
                    "run_best_macro_f1": best_quality,
                    ...
                    "macro_recall": float(row["test_macro_recall"]),
                    "rms_tpr_gap": float(row["test_rms_tpr_gap"]),
                    "utility_penalty": float(
                        row["test_utility_penalty"]
                    ),
                    "max_abs_tpr_gap": float(
                        row["test_max_abs_tpr_gap"]
                    ),
                    "mean_signed_tpr_gap": float(
                        row["test_mean_signed_tpr_gap"]
                    ),
                    "valid_professions": int(
                        row["test_valid_professions"]
                    ),
                    "max_ci_width": float(
                        row["test_max_ci_width"]
                    ),
                    "prompt_id": row.get(
                        "prompt_id",
                        "",
    ...
    if len(points) != expected_rows:
        raise RuntimeError(
            f"Expected {expected_rows} high-quality operating points, "
            f"found {len(points)}"
        )
    return points
...
def plot_high_quality_operating_points(
    ...
    *,
    ...
    points = high_quality_operating_points(
        final,
        fraction=fraction,
    )
    points.to_csv(
        outdir
        / "bias_in_bios_qwen3_5m_v2_high_quality_operating_points.csv",
        index=False,
    )
    ...
    # The utility-valid macro-recall threshold is fixed by the experiment.
    axes[1, 1].axhline(
        0.20,
        color="0.5",
        linestyle="--",
    ...
    axes[0, 0].legend(frameon=False)
    fig.suptitle(
        "Bias-in-Bios v2 — Lowest Native Unfairness "
        f"within {100.0 * fraction:.0f}% of Run-Best Macro-F1"
    )
    save_figure(
        fig,
        outdir,
        "bias_in_bios_qwen3_5m_v2_high_quality_operating_points",
    )
    return points
...
def write_summary_tables(
    ...
    high_quality_points: pd.DataFrame,
    outdir: Path,
) -> None:
    diagnostics = utility_validity_summary(
        final_raw_evaluations
    )
    best_points = attach_bios_diagnostics(
        best_quality_points(
            final_evaluations
        )
    )
    ...
    for optimizer in OPTIMIZER_ORDER:
        ...
        valid_eval = final_evaluations[
            final_evaluations["optimizer"] == optimizer
        ]
        raw_eval = final_raw_evaluations[
            final_raw_evaluations["optimizer"] == optimizer
        ]
        best = best_points[
            best_points["optimizer"] == optimizer
        ]
        high = high_quality_points[
            high_quality_points["optimizer"] == optimizer
        ]
        ...
        rows.append(
            {
                "Method": DISPLAY_NAME[optimizer],
                "Seeds": int(
                    metrics["seed"].nunique()
                ),
                "Raw 5M candidates": int(
                    len(raw_eval)
                ),
                "Dev/test-ready 5M candidates": int(
                    len(valid_eval)
                ),
                "Macro-recall-valid fraction mean": diagnostic_method[
                    "macro_recall_valid_fraction"
                ].mean(),
                "Actual tokens mean": metrics[
                    "actual_budget_tokens"
                ].mean(),
                "Holdout nR2 mean ↓": metrics[
                    "noisy_r2_3d"
                ].mean(),
                "Holdout nR2 SD": metrics[
                    "noisy_r2_3d"
                ].std(ddof=1),
                "HV optimistic mean ↑": metrics[
                    "hv_test_optimistic_3d"
                ].mean(),
                "HV pessimistic mean ↑": metrics[
                    "hv_test_pessimistic_3d"
                ].mean(),
                "Gap mean ↓": metrics[
                    "approximation_gap_3d"
                ].mean(),
                "Balanced test macro-F1 mean ↑": metrics[
                    "balanced_test_quality"
                ].mean(),
                "Balanced cost objective mean ↓": metrics[
                    "balanced_test_cost"
                ].mean(),
                "Balanced native unfairness mean ↓": metrics[
                    "balanced_test_fairness"
                ].mean(),
                "Best-quality mean macro-F1 ↑": best[
                    "test_quality"
                ].mean(),
                "Best-quality mean cost ↓": best[
                    "test_cost"
                ].mean(),
                "Best-quality mean native unfairness ↓": best[
                    "test_fairness"
                ].mean(),
                "Best-quality mean macro recall ↑": best[
                    "test_macro_recall"
                ].mean(),
                "Best-quality mean RMS TPR gap ↓": best[
                    "test_rms_tpr_gap"
                ].mean(),
                "Best-quality mean max |TPR gap| ↓": best[
                    "test_max_abs_tpr_gap"
                ].mean(),
                "High-quality mean macro-F1 ↑": high[
                    "test_quality"
                ].mean(),
                "High-quality mean cost ↓": high[
                    "test_cost"
                ].mean(),
                "High-quality mean native unfairness ↓": high[
                    "test_fairness"
                ].mean(),
                "High-quality mean macro recall ↑": high[
                    "macro_recall"
                ].mean(),
                "High-quality mean RMS TPR gap ↓": high[
                    "rms_tpr_gap"
                ].mean(),
                "High-quality mean quality retention": high[
                    "quality_retention"
                ].mean(),
            }
        )
    ...
    table.to_csv(
        outdir
        / "bias_in_bios_qwen3_5m_v2_summary_table.csv",
        index=False,
    )
    (
        outdir
        / "bias_in_bios_qwen3_5m_v2_summary_table.md"
    ).write_text(
        table.to_markdown(
            index=False,
            floatfmt=".4f",
    ...
    diagnostics.to_csv(
        outdir
        / "bias_in_bios_qwen3_5m_v2_validity_summary.csv",
        index=False,
    )
    (
        outdir
        / "bias_in_bios_qwen3_5m_v2_validity_summary.md"
    ).write_text(
    ...
    print(
        "\nBias-in-Bios v2 5M method summary"
    )
    print(
...
def write_readme(
    ...
    *,
    ...
    content = f"""# Bias-in-Bios v2 / Qwen-3-30B / 5M publication figures

Generated by `python -m analysis.make_bias_in_bios_5m_figures_v2`.
...
- Publication metrics require both `dev_fairness_ready` and `test_fairness_ready`.
- Macro-recall-invalid candidates are retained because their shortfall is already folded into the native unfairness objective through `utility_penalty`.
- Fixed objective bounds: quality loss `[0,1]`, cost `[0,{cost_upper_bound:g}]`, unfairness `[0,1]`.
- High-quality operating points retain at least `{100.0 * high_quality_fraction:.0f}%` of each run's best macro-F1.

## Native quality and fairness objectives

Bias-in-Bios v2 uses macro-F1 as the quality objective.

For fairness, smoothed true-positive rates are computed separately for male and
female biographies inside each of the 28 professions. The signed gender TPR
gaps are RMS aggregated. The optimized unfairness is the larger of:

1. the profession-conditional gender TPR-gap RMS; and
2. the normalized macro-recall shortfall below 0.20.

Wilson confidence-interval widths determine whether all 28 profession estimates
are statistically ready. Utility-penalized rows are retained because the penalty
is part of the objective rather than an after-the-fact exclusion rule.
...
- Exact holdout metrics are shown only for checkpoints represented by all six method/seed runs. The current direct-to-5M study provides the exact 5M checkpoint.
...
- `bias_in_bios_qwen3_5m_v2_stepwise_dev_nr2_proxy.*`: development-only anytime proxy, not exact holdout nR2.
- Raw and readiness-valid evaluation tables are both retained for auditability.

## Main figure files

- `bias_in_bios_qwen3_5m_v2_stepwise_dev_nr2_proxy.*`
- `bias_in_bios_qwen3_5m_v2_stepwise_readiness_utility.*`
- `bias_in_bios_qwen3_5m_v2_stepwise_hv_exact_gap.*`
- `bias_in_bios_qwen3_5m_v2_exact_nr2_hv_gap.*`
- `bias_in_bios_qwen3_5m_v2_balanced_exact_metrics.*`
- `bias_in_bios_qwen3_5m_v2_attainment_macro_f1_cost.*`
- `bias_in_bios_qwen3_5m_v2_attainment_macro_f1_unfairness.*`
- `bias_in_bios_qwen3_5m_v2_pareto_cost_unfairness.*`
- `bias_in_bios_qwen3_5m_v2_test_pareto_3d.*`
- `bias_in_bios_qwen3_5m_v2_method_comparison_hv_nr2_gap.*`
- `bias_in_bios_qwen3_5m_v2_fairness_diagnostics.*`
- `bias_in_bios_qwen3_5m_v2_high_quality_operating_points.*`
- `bias_in_bios_qwen3_5m_v2_trifair_fewshot_outputshare.*`
- `bias_in_bios_qwen3_5m_v2_trifair_fewshot_unfairness.*`
- `bias_in_bios_qwen3_5m_v2_trifair_fewshot_macro_recall.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    reference_point_values = tuple(
        float(piece.strip())
        for piece in str(args.reference_point).split(",")
    )
    if len(reference_point_values) != 3:
        raise ValueError(
            "--reference-point must contain exactly three values"
        )
    reference_point = (
    ...
    cost_upper_bound = float(
        args.cost_upper_bound
    )
    high_quality_fraction = float(
        args.high_quality_fraction
    )
    ...
    needs_rebuild = (
        args.rebuild_analysis
        or not analysis_has_5m(
            analysis_dir,
            results_root=results_root,
            bounds_file=bounds_file,
            cost_upper_bound=cost_upper_bound,
    ...
    if needs_rebuild:
        if (
            args.no_auto_rebuild
            and not args.rebuild_analysis
        ):
            raise RuntimeError(
    ...
    (
        ...
        evaluations,
        raw_evaluations,
    ) = read_inputs(analysis_dir)

    selected = selected_final_runs(
        run_metrics
    )
    final_evaluations = (
        final_checkpoint_evaluations(
            evaluations,
            selected,
        )
    )
    ...
    checkpoint_metrics = (
        complete_checkpoint_metrics(
            checkpoint_run_metrics(
                run_metrics
            )
        )
    )
    ...
    selected_trajectory = (
        filter_to_selected_runs(
            trajectory,
            selected,
        )
    )
    step_results = clean_frame(
        load_step_results_from_selected_runs(
            selected
        )
    )
    step_proxy = (
        stepwise_development_r2_proxy(
            step_results,
            final_evaluations,
            cost_upper_bound=cost_upper_bound,
        )
    ...
    selected.to_csv(
        figure_dir
        / "bias_in_bios_qwen3_5m_v2_selected_runs.csv",
        index=False,
    )
    checkpoint_metrics.to_csv(
        figure_dir
        / "bias_in_bios_qwen3_5m_v2_checkpoint_run_metrics.csv",
        index=False,
    )
    final_evaluations.to_parquet(
        figure_dir
        / "bias_in_bios_qwen3_5m_v2_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir
        / "bias_in_bios_qwen3_5m_v2_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
        figure_dir
        / "bias_in_bios_qwen3_5m_v2_stepwise_dev_nr2_proxy.csv",
        index=False,
    )
    ...
    plot_stepwise_readiness_and_utility(
        step_proxy,
        figure_dir,
        max_budget=max_plot_budget,
    )
    ...
    plot_exact_checkpoint_mo_metrics(
        checkpoint_metrics,
        figure_dir,
    )
    plot_balanced_budget_trajectory(
        checkpoint_metrics,
        figure_dir,
    )
    plot_empirical_attainment(
        ...
        filename=(
            "bias_in_bios_qwen3_5m_v2_"
            "attainment_macro_f1_cost"
        ),
    )
    plot_empirical_attainment(
        final_evaluations,
        ...
        filename=(
            "bias_in_bios_qwen3_5m_v2_"
            "attainment_macro_f1_unfairness"
        ),
    )
    ...
    plot_fairness_validity_diagnostics(
        final_raw_evaluations,
        figure_dir,
    )
    high_quality_points = (
        plot_high_quality_operating_points(
            final_evaluations,
            figure_dir,
            fraction=high_quality_fraction,
        )
    )
    ...
    plot_trifair_diagnostic(
        ...
        filename=(
            "bias_in_bios_qwen3_5m_v2_"
            "trifair_fewshot_unfairness"
        ),
    )
    plot_trifair_diagnostic(
        attach_bios_diagnostics(
            final_evaluations
        ),
        figure_dir,
        color_col="test_macro_recall",
        color_label="Macro Recall ↑",
        filename=(
            "bias_in_bios_qwen3_5m_v2_"
            "trifair_fewshot_macro_recall"
        ),
    )
    ...
    print(
        "\nBias-in-Bios v2 5M figures "
        f"written to: {figure_dir.resolve()}"
    )
