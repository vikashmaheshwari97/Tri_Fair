"""Create publication-style Civil Comments 5M figures for the final Tri-Fair-v7 study.

This is the Qwen3-30B / Civil Comments adaptation of
...
Safeguards
----------
1. Exactly the intended six Civil Comments optimisation runs are accepted.
...
Default Rocket usage::
    ...
    python -m analysis.make_civil_comments_5m_figures_qwen_v7 --rebuild-analysis --strict
...
UNFAIRNESS_AXIS_LABEL = "Equalized-Odds Unfairness ↓"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/civil_comments_qwen3_30b_v7_final5m",
        help="Dedicated Qwen3-30B Civil Comments analysis-table directory.",
    )
    parser.add_argument(
        "--figure-dir",
        default=None,
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_civil_comments_qwen3_30b_v7_final5m.json",
        help="Dedicated deterministic Qwen3-30B v7 Civil Comments bounds file; incompatible legacy bounds are rejected.",
    )
    parser.add_argument(
        "--cost-upper-bound",
        type=float,
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended final Qwen3-30B / Civil Comments / v7 rows."""
    out = frame.copy()
    if "run_dir" in out:
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="civil_comments_qwen3_30b_v7_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
...
def _load_shared_union_final_evaluations(
    ...
    *,
    ...
    if errors:
        ...
        warnings.warn(message)
    if not frames:
        raise RuntimeError("No final shared-union Civil Comments evaluations were loaded")
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild final Civil Comments analysis from shared-union holdout and six run histories."""
    del budgets
    ...
    print(f"\nRebuilding Qwen3-30B Civil Comments v7 5M tables from: {results_root.resolve()}")
...
def plot_nr2_development_and_holdout(
    ...
    *,
    ...
    fig.suptitle(
        "Civil Comments — Qwen3-30B: Development Dynamics and Final Holdout",
        fontsize=12,
    )
    save_figure(
        fig,
        outdir,
        "civil_comments_qwen3_30b_v7_5m_final_nr2_development_and_holdout",
    )
...
def plot_stepwise_coverage_validity(
    ...
    *,
    ...
    ax.set_title("Civil Comments — Development Fairness Validity")
    ax.set_xlabel("Cumulative Downstream Tokens [×10⁶]")
    ...
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_stepwise_coverage_validity")
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("Civil Comments — Qwen3-30B")
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments — Qwen3-30B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_exact_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments — Qwen3-30B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Civil Comments — Qwen3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Civil Comments — Qwen3-30B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Accuracy ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments — Qwen3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Civil Comments — Qwen3-30B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Accuracy")
    ax.grid(True, alpha=0.25)
...
def plot_coverage_validity_diagnostics(final_raw: pd.DataFrame, outdir: Path) -> None:
    ...
    summary.to_csv(
        outdir / "civil_comments_qwen3_30b_v7_5m_final_coverage_validity_by_seed.csv", index=False
    )
    ...
    fig.suptitle("Civil Comments — Qwen3-30B at 5M")
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_coverage_validity_diagnostics")
...
def plot_high_accuracy_operating_points(
    ...
    *,
    ...
    points.to_csv(
        outdir / "civil_comments_qwen3_30b_v7_5m_final_high_accuracy_operating_points.csv",
        index=False,
    )
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 3.9), constrained_layout=True)
    specifications = [
    ...
    save_figure(fig, outdir, "civil_comments_qwen3_30b_v7_5m_final_high_accuracy_operating_points")
    return points
...
def write_summary_tables(
    ...
    table.to_csv(outdir / "civil_comments_qwen3_30b_v7_5m_final_summary_table.csv", index=False)
    (outdir / "civil_comments_qwen3_30b_v7_5m_final_summary_table.md").write_text(
        table.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    coverage.to_csv(
        outdir / "civil_comments_qwen3_30b_v7_5m_final_coverage_summary.csv", index=False
    )
    (outdir / "civil_comments_qwen3_30b_v7_5m_final_coverage_summary.md").write_text(
...
def write_readme(
    ...
    *,
    ...
    content = f"""# Civil Comments / Qwen3-30B / Tri-Fair-v7 / 5M publication figures

Generated by `python -m analysis.make_civil_comments_5m_figures_qwen_v7`.
...
- `civil_comments_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`: Panel A is the development-only anytime proxy; Panel B shows the exact 5M holdout nR2 for seeds 72, 73, and 74 with method mean ± SD in the legend.
...
- `civil_comments_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`
- `civil_comments_qwen3_30b_v7_5m_final_stepwise_coverage_validity.*`
- `civil_comments_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap.*`
- `civil_comments_qwen3_30b_v7_5m_final_exact_nr2_hv_gap.*`
- `civil_comments_qwen3_30b_v7_5m_final_balanced_exact_metrics.*`
- `civil_comments_qwen3_30b_v7_5m_final_attainment_accuracy_cost.*`
- `civil_comments_qwen3_30b_v7_5m_final_attainment_accuracy_unfairness.*`
- `civil_comments_qwen3_30b_v7_5m_final_pareto_cost_unfairness.*`
- `civil_comments_qwen3_30b_v7_5m_final_test_pareto_3d.*`
- `civil_comments_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap.*`
- `civil_comments_qwen3_30b_v7_5m_final_coverage_validity_diagnostics.*`
- `civil_comments_qwen3_30b_v7_5m_final_high_accuracy_operating_points.*`
- `civil_comments_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare.*`
- `civil_comments_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    selected.to_csv(
        figure_dir / "civil_comments_qwen3_30b_v7_5m_final_selected_runs.csv", index=False
    )
    checkpoint_metrics.to_csv(
        figure_dir / "civil_comments_qwen3_30b_v7_5m_final_checkpoint_run_metrics.csv", index=False
    )
    final_evaluations.to_parquet(
        figure_dir / "civil_comments_qwen3_30b_v7_5m_final_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir / "civil_comments_qwen3_30b_v7_5m_final_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
        figure_dir / "civil_comments_qwen3_30b_v7_5m_final_stepwise_dev_nr2_proxy.csv", index=False
    )
    ...
    plot_empirical_attainment(
        ...
        filename="civil_comments_qwen3_30b_v7_5m_final_attainment_accuracy_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_30b_v7_5m_final_attainment_accuracy_unfairness",
    )
    ...
    plot_trifair_diagnostic(
        ...
        filename="civil_comments_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness",
    )
