"""Create publication-style Bias-in-Bios 5M figures for the final Tri-Fair-v7 study.

This is the Qwen3-30B / Bias-in-Bios adaptation of
``make_bbq_5m_figures_gptoss.py``.  It intentionally preserves the reference
script's figure set, panel layouts, black/orange visual language, markers,
step trajectories, and uncertainty bands.  It does not introduce additional
figures or synthetic checkpoints.

The final study is stored under ``results/tri_fair_v7_qwen_5m`` and contains:

* Tri-Fair-v7 and NSGA-II-PO-Fair;
* seeds 72, 73, and 74;
* a strict 5M downstream-token budget;
* deterministic shared-union held-out evaluation.

Safeguards
----------
1. Exactly the intended six Bias-in-Bios optimisation runs are accepted.
2. The shared-union held-out file is the sole source of test metrics, so an
   identical prompt has one held-out result regardless of method membership.
3. The final real state for each method/seed is promoted to the configured 5M
   checkpoint only after its run summary is validated.
4. Main figures require ``test_fairness_ready=True``. When the fairness
   diagnostic supplies ``coverage_valid``, that flag is also respected.
   Invalid candidates remain only in the validity diagnostic and audit tables.
5. Objective normalization is fixed at quality loss [0,1], weighted mean-token
   cost [0,120], and unfairness [0,1].
6. Development trajectories are computed only from the six frozen final-study
   run histories.  The seed-71 screening namespace is never mixed in.
7. The cost objective is ``0.11 × mean input tokens + 0.41 × mean output
   tokens``; it is not a currency amount or Rocket GPU charge.

Default Rocket usage::

    python -m analysis.make_bias_in_bios_5m_figures_qwen_v7 --rebuild-analysis --strict

The generated figure set is exactly the same as the uploaded GPT-OSS reference script.
"""
...
UNFAIRNESS_AXIS_LABEL = "TPR-Gap Unfairness ↓"

OPTIMIZER_ORDER = ["Tri-Fair-v7", "NSGAII-PO-Fair"]
DISPLAY_NAME = {
    "Tri-Fair-v7": "Tri-Fair",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--results-root",
        default="results/tri_fair_v7_qwen_5m",
        help="Root containing the final fresh Qwen3-30B 5M result namespace.",
    )
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/bias_in_bios_qwen3_30b_v7_final5m",
        help="Dedicated Qwen3-30B Bias-in-Bios analysis-table directory.",
    )
    parser.add_argument(
        "--figure-dir",
        default=None,
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_bias_in_bios_qwen3_30b_v7_final5m.json",
        help="Dedicated deterministic Qwen3-30B v7 Bias-in-Bios bounds file; incompatible legacy bounds are rejected.",
    )
    parser.add_argument(
        "--cost-upper-bound",
        type=float,
        default=FIXED_COST_UPPER_BOUND,
        help="Fixed cost-objective upper bound used by the Bias-in-Bios configuration.",
    )
    parser.add_argument(
        "--overwrite-bounds",
        action="store_true",
        help="Replace an incompatible dedicated Qwen3-30B v7 bounds file.",
    )
    parser.add_argument(
        "--budgets",
    ...
    parser.add_argument(
        "--high-macro-f1-threshold",
        dest="high_accuracy_threshold",
        type=float,
        default=HIGH_ACCURACY_THRESHOLD,
        help="Macro-F1 threshold for the high-performance fairness operating point.",
    )
    parser.add_argument(
        "--rebuild-analysis",
        action="store_true",
        help="Force rebuilding the dedicated Qwen3-30B v7 analysis tables.",
    )
    parser.add_argument(
        "--no-auto-rebuild",
        action="store_true",
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Raise on unreadable artifacts or missing fairness-readiness fields.",
    )
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended final Qwen3-30B / Bias-in-Bios / v7 rows."""
    out = frame.copy()
    if "run_dir" in out:
        run_dir = out["run_dir"].astype(str)
        excluded = (
            run_dir.str.contains("screen", case=False, regex=False)
            | run_dir.str.contains("pilot", case=False, regex=False)
            | run_dir.str.contains("archive", case=False, regex=False)
            | run_dir.str.contains("tri_fair_v6", case=False, regex=False)
        )
        out = out[~excluded]
    if "dataset" in out:
...
def promote_strict_final_evaluations(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            "Qwen3-30B v7 final-evaluation table lacks checkpoint fields: "
            + ", ".join(missing)
        )
...
def attach_dataset_validity(frame: pd.DataFrame) -> pd.DataFrame:
    """Attach fairness readiness and any available coverage-validity flag.

    Civil Comments and Bias-in-Bios do not require the BBQ-specific
    ``coverage_valid`` diagnostic. When a metric implementation supplies that
    key, it is respected; otherwise fairness readiness is the validity rule.
    """
    out = frame.copy()
    for split in ("dev", "test"):
        ...
        if diagnostics in out:
            parsed = out[diagnostics].map(_json_object)
            has_coverage = parsed.map(
                lambda value: "coverage_valid" in value
            )
            reported_coverage = parsed.map(
                lambda value: bool(
                    value.get("coverage_valid", False)
                )
            )
            out[f"{split}_coverage_valid"] = np.where(
                has_coverage,
                reported_coverage,
                ready,
            ).astype(bool)
        else:
            out[f"{split}_coverage_valid"] = ready.astype(bool)
    ...
    out["publication_valid"] = (
        test_ready & out["test_coverage_valid"]
    )
...
def publication_valid_evaluations(
    ...
    *,
    ...
    """Keep rows with finite, fairness-ready held-out objectives."""
    enriched = attach_dataset_validity(frame)
    if strict and "test_fairness_ready" not in enriched:
        raise ValueError(
            "Evaluation table lacks required test_fairness_ready values"
        )
    valid = enriched[enriched["publication_valid"]].copy()
    if valid.empty:
        raise RuntimeError(
            "No publication-valid rows remain after requiring held-out "
            "fairness readiness and any available coverage-validity flag"
        )
...
def ensure_qwen_bounds(
    path: Path,
    *,
    cost_upper_bound: float,
    overwrite: bool,
):
    """Create or validate deterministic Bias-in-Bios normalization bounds."""
    ...
    if path.is_file() and not overwrite:
        ...
        if not (
            ...
            raise RuntimeError(
                ...
                f"and maximum={expected_max.tolist()}. Use the dedicated Qwen file or "
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="bias_in_bios_qwen3_30b_v7_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
    print(f"Wrote deterministic Bias-in-Bios bounds: {path.resolve()}")
    return store
...
def write_analysis_manifest(
    ...
    *,
    ...
    payload = {
        ...
        "validity_rule": "test_fairness_ready and coverage_valid when supplied by the metric diagnostic",
...
def _run_key(optimizer: str, seed: int, run_dir: Path) -> str:
    return f"{MODEL}/{DATASET}/{optimizer}/seed{seed}/{run_dir.name}"
...
def _resolve_run_dir(
    ...
    optimizer: str,
    seed: int,
    source_log_path: object,
) -> Path:
    source = Path(str(source_log_path)).expanduser()
    if source.is_dir() and (source / "run_summary.json").is_file():
        return source.resolve()

    base = results_root / MODEL / DATASET / optimizer / f"seed{seed}"
    candidates = sorted(path.parent for path in base.glob("*/run_summary.json"))
    if len(candidates) != 1:
        raise RuntimeError(
            f"Expected one final run directory for {optimizer}/seed{seed}, "
            f"found {len(candidates)} beneath {base}"
        )
...
def _validate_final_run(
    ...
    *,
    ...
    controller = summary.get("budget_controller") or {}
    if not isinstance(controller, dict):
        raise ValueError(f"{run_dir}: budget_controller is not a JSON object")

    status = str(summary.get("status") or "")
    requested = int(controller.get("requested_budget_tokens", 0) or 0)
    actual = int(controller.get("actual_downstream_tokens", 0) or 0)
    utilization = float(controller.get("budget_utilization", float("nan")))
    strict_enabled = bool(controller.get("strict_budget_enabled", False))
    ...
    valid_stop = (
        status == "complete"
        and requested == FINAL_BUDGET
        and 0 < evaluated_tokens <= actual <= FINAL_BUDGET
        and np.isfinite(utilization)
        and MIN_STRICT_FINAL_UTILIZATION <= utilization <= 1.0 + 1e-12
        and (
            actual >= FINAL_BUDGET
    ...
    if not valid_stop:
        raise RuntimeError(
            f"Invalid final run {optimizer}/seed{seed}: status={status!r}, "
            f"requested={requested}, evaluated={evaluated_tokens}, actual={actual}, "
            f"utilization={utilization}, strict={strict_enabled}, reason={reason!r}"
        )
    return {
        "summary_status": status,
        "summary_actual_tokens": actual,
        "summary_utilization": utilization,
        "summary_stopping_reason": reason,
        "strict_budget_enabled": strict_enabled,
    }


def _load_shared_union_final_evaluations(
    results_root: Path,
    *,
    strict: bool,
) -> pd.DataFrame:
    ...
    for seed in EXPECTED_SEEDS:
        ...
        if not path.is_file():
            errors.append(f"Missing shared-union file: {path}")
            continue
        ...
        # The final-state holdout jobs may include more than one real state in
        # the 4.5M--5M interval.  Select the last real state independently for
        # each method, exactly as the final-study analysis did.
        for optimizer in OPTIMIZER_ORDER:
            method = frame[frame["optimizer"] == optimizer].copy()
            if method.empty:
                errors.append(f"{path}: no rows for {optimizer}")
                continue
            tokens = pd.to_numeric(
                method["actual_budget_tokens"], errors="coerce"
            )
            if not tokens.notna().any():
                errors.append(f"{path}: no finite token state for {optimizer}")
                continue
            final_tokens = int(tokens.max())
            method = method[tokens == final_tokens].copy()
            run_dir = _resolve_run_dir(
                results_root,
                optimizer,
                seed,
                method["source_log_path"].iloc[0],
            )
            audit = _validate_final_run(
                run_dir,
                optimizer=optimizer,
                seed=seed,
                evaluated_tokens=final_tokens,
            )
            method["run_dir"] = str(run_dir)
            method["run_key"] = _run_key(optimizer, seed, run_dir)
            method["model"] = MODEL
            method["dataset"] = DATASET
            method["seed"] = int(seed)
            method["configured_budget"] = FINAL_BUDGET
            method["budget_checkpoint"] = FINAL_BUDGET
            method["strict_final_checkpoint_promoted"] = final_tokens < FINAL_BUDGET
            for key, value in audit.items():
                method[key] = value
            method["shared_union_file"] = str(path.resolve())
            frames.append(method)

    if errors:
        message = "Shared-union final evaluation validation failed:\n" + "\n".join(errors)
        if strict:
            raise RuntimeError(message)
        warnings.warn(message)
    if not frames:
        raise RuntimeError("No final shared-union Bias-in-Bios evaluations were loaded")
    ...
    observed = {
        (str(row.optimizer), int(row.seed))
        for row in output[["optimizer", "seed"]]
        .drop_duplicates()
        .itertuples(index=False)
    }
    if observed != expected:
        raise RuntimeError(
            f"Final shared-union grid mismatch: missing={sorted(expected-observed)}, "
...
def _compute_run_metrics_from_shared_union(
    valid_evaluations: pd.DataFrame,
    bounds_store,
    *,
    n_preferences: int,
    preference_seed: int,
    ...
    from analysis.metrics import analyse_evaluated_run  # type: ignore

    bounds = bounds_store[(DATASET, MODEL)]
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            group = valid_evaluations[
                (valid_evaluations["optimizer"] == optimizer)
                & (pd.to_numeric(valid_evaluations["seed"], errors="coerce") == seed)
            ].copy()
            if group.empty:
                raise RuntimeError(
                    f"No publication-valid final rows for {optimizer}/seed{seed}"
                )
            ...
            rows.append(
                {
                    ...
                    "seed": int(seed),
                    "chosen_step": int(
                        pd.to_numeric(group["chosen_step"], errors="coerce").max()
                    ),
                    ...
                    "configured_budget": FINAL_BUDGET,
                    **metrics,
                }
            )
...
def _compute_development_trajectory(
    run_metrics: pd.DataFrame,
    bounds_store,
    *,
    reference_point: tuple[float, float, float],
) -> pd.DataFrame:
    from analysis.metrics import hypervolume_3d  # type: ignore
    from analysis.objectives import pareto_mask  # type: ignore
    ...
    for row in selected.itertuples(index=False):
        ...
        if not path.is_file():
            raise FileNotFoundError(path)
        steps = pd.read_parquet(path).copy()
        require_columns(
            steps,
            ["step", "quality", "cost", "fairness"],
            str(path),
        )
        for step, group in steps.groupby("step", sort=True):
            quality = finite_numeric(group["quality"])
            cost = finite_numeric(group["cost"])
            fairness = finite_numeric(group["fairness"])
            valid = np.isfinite(quality) & np.isfinite(cost) & np.isfinite(fairness)
            if "fairness_ready" in group:
                valid &= group["fairness_ready"].fillna(False).astype(bool).to_numpy()
            if not valid.any():
                continue
            raw = np.column_stack(
                [1.0 - quality[valid], cost[valid], fairness[valid]]
            )
            normalized = bounds.normalize(raw, clip=False)
            front = normalized[pareto_mask(normalized)]
            if "total_tokens_downstream" in group:
                actual_tokens = int(
                    np.nanmax(finite_numeric(group["total_tokens_downstream"]))
                )
            else:
                actual_tokens = int(
            ...
            rows.append(
                {
                    ...
                    "budget_checkpoint": (
                        FINAL_BUDGET if actual_tokens >= FINAL_BUDGET else actual_tokens
                    ),
                    "hv_dev_3d": hypervolume_3d(front, reference_point),
                    "dev_front_size": int(len(front)),
                }
            )
    if not rows:
        raise RuntimeError("No development trajectory rows were computed")
    return pd.DataFrame(rows)
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild final Bias-in-Bios analysis from shared-union holdout and six run histories."""
    del budgets
    if not results_root.is_dir():
        raise FileNotFoundError(
            f"Results root does not exist: {results_root}\n"
            "Expected results/tri_fair_v7_qwen_5m or pass --results-root."
        )

    from analysis.metrics import aggregate_run_metrics  # type: ignore

    print(f"\nRebuilding Qwen3-30B Bias-in-Bios v7 5M tables from: {results_root.resolve()}")
    raw_evaluations = _load_shared_union_final_evaluations(
        results_root,
        strict=strict,
    )
    raw_evaluations = attach_dataset_validity(raw_evaluations)
    valid_evaluations = publication_valid_evaluations(
        raw_evaluations,
        strict=strict,
    )

    bounds_store = ensure_qwen_bounds(
        bounds_file,
        cost_upper_bound=cost_upper_bound,
        overwrite=overwrite_bounds,
    )
    run_metrics = _compute_run_metrics_from_shared_union(
        valid_evaluations,
        bounds_store,
        n_preferences=n_preferences,
        preference_seed=preference_seed,
    ...
    trajectory = _compute_development_trajectory(
        run_metrics,
        bounds_store,
        reference_point=reference_point,
    )
    ...
    raw_evaluations.to_parquet(
        analysis_dir / "all_evaluations_raw.parquet",
        index=False,
    )
    valid_evaluations.to_parquet(
        analysis_dir / "all_evaluations_valid.parquet",
        index=False,
    )
...
def read_inputs(
    ...
    if missing:
        raise FileNotFoundError(
            "Missing required Bias-in-Bios analysis outputs:\n"
            + "\n".join(missing)
    ...
    raw_evaluations = attach_dataset_validity(
...
def infer_cost_bounds(
    ...
    *,
    ...
    """Return the fixed Bias-in-Bios cost bound; data-dependent rescaling is forbidden."""
    ...
    for keys, group in step_results.groupby(group_cols, sort=True, dropna=False):
        ...
        if "fairness_diagnostics_json" in group:
            ...
            coverage_fraction = float(
                np.mean(coverage_valid[valid_mask])
            )
        else:
...
def plot_nr2_development_and_holdout(
    ...
    *,
    ...
    # We deliberately do not connect seeds because 72, 73 and 74 are categorical
    ...
    fig.suptitle(
        "Bias-in-Bios — Qwen3-30B: Development Dynamics and Final Holdout",
        fontsize=12,
    )
    save_figure(
        fig,
        outdir,
        "bias_in_bios_qwen3_30b_v7_5m_final_nr2_development_and_holdout",
    )
...
def plot_stepwise_coverage_validity(
    ...
    *,
    ...
    ax.set_title("Bias-in-Bios — Development Fairness Validity")
    ax.set_xlabel("Cumulative Downstream Tokens [×10⁶]")
    ax.set_ylabel("Fairness-Valid Candidate Fraction")
    ax.set_ylim(0.0, 1.05)
    ax.set_xlim(left=0.0, right=max_budget / 1_000_000.0)
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_stepwise_coverage_validity")
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("Bias-in-Bios — Qwen3-30B")
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Bias-in-Bios — Qwen3-30B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_exact_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Bias-in-Bios — Qwen3-30B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Bias-in-Bios — Qwen3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Bias-in-Bios — 5M Cost vs Test Unfairness")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Bias-in-Bios — Qwen3-30B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Macro-F1 ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Bias-in-Bios — Qwen3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap")
...
def trifair_final_candidates(final: pd.DataFrame) -> pd.DataFrame:
    data = final[final["optimizer"] == "Tri-Fair-v7"].copy()
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Bias-in-Bios — Qwen3-30B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Macro-F1")
    ax.grid(True, alpha=0.25)
...
def coverage_validity_summary(final_raw: pd.DataFrame) -> pd.DataFrame:
    data = attach_dataset_validity(final_raw)
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            group = data[
...
def plot_coverage_validity_diagnostics(final_raw: pd.DataFrame, outdir: Path) -> None:
    ...
    summary.to_csv(
        outdir / "bias_in_bios_qwen3_30b_v7_5m_final_coverage_validity_by_seed.csv", index=False
    )
    ...
    axes[0].set_ylabel("Test Fairness-Valid Fraction")
    axes[0].set_ylim(0.0, 1.05)
    axes[0].set_title("Fairness-Valid Candidate Rate")
    ...
    axes[1].set_title("Why Fairness Validity Matters")
    axes[1].grid(True, alpha=0.25)
    axes[1].legend(frameon=False, fontsize=8)
    fig.suptitle("Bias-in-Bios — Qwen3-30B at 5M")
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_coverage_validity_diagnostics")
...
def high_macro_f1_operating_points(
    final: pd.DataFrame,
    *,
    threshold: float,
) -> pd.DataFrame:
    if not 0.0 <= threshold <= 1.0:
        raise ValueError("--high-macro-f1-threshold must be between 0 and 1")
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            group = final[
...
def plot_high_macro_f1_operating_points(
    final: pd.DataFrame,
    outdir: Path,
    *,
    threshold: float,
) -> pd.DataFrame:
    points = high_macro_f1_operating_points(final, threshold=threshold)
    points.to_csv(
        outdir / "bias_in_bios_qwen3_30b_v7_5m_final_high_macro_f1_operating_points.csv",
        index=False,
    )
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 3.9), constrained_layout=True)
    specifications = [
        ("test_quality", "Macro-F1 ↑", "Test Macro-F1"),
    ...
    axes[0].legend(frameon=False)
    fig.suptitle(
        f"Bias-in-Bios — Lowest Unfairness with Test Macro-F1 ≥ {threshold:.2f}"
    )
    save_figure(fig, outdir, "bias_in_bios_qwen3_30b_v7_5m_final_high_macro_f1_operating_points")
    return points
        ...
        rows.append(
            {
                ...
                "Fairness-valid fraction mean": coverage_method[
                    "test_coverage_valid_fraction"
                ].mean(),
                ...
                "Balanced test macro-F1 mean ↑": metrics["balanced_test_quality"].mean(),
                "Balanced cost objective mean ↓": metrics["balanced_test_cost"].mean(),
                "Balanced test unfairness mean ↓": metrics[
                    "balanced_test_fairness"
                ].mean(),
                "Best-quality point mean macro-F1 ↑": best["test_quality"].mean(),
                "Best-quality point mean cost ↓": best["test_cost"].mean(),
                "Best-quality point mean unfairness ↓": best[
                    "test_fairness"
                ].mean(),
                "High-performance point mean macro-F1 ↑": high["test_quality"].mean(),
                "High-performance point mean cost ↓": high["test_cost"].mean(),
                "High-performance point mean unfairness ↓": high[
                    "test_fairness"
                ].mean(),
            }
        )
    table = pd.DataFrame(rows)
    table.to_csv(outdir / "bias_in_bios_qwen3_30b_v7_5m_final_summary_table.csv", index=False)
    (outdir / "bias_in_bios_qwen3_30b_v7_5m_final_summary_table.md").write_text(
        table.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    coverage.to_csv(
        outdir / "bias_in_bios_qwen3_30b_v7_5m_final_coverage_summary.csv", index=False
    )
    (outdir / "bias_in_bios_qwen3_30b_v7_5m_final_coverage_summary.md").write_text(
        coverage.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    print("\nBias-in-Bios 5M method summary")
...
def write_readme(
    ...
    *,
    ...
    content = f"""# Bias-in-Bios / Qwen3-30B / Tri-Fair-v7 / 5M publication figures

Generated by `python -m analysis.make_bias_in_bios_5m_figures_qwen_v7`.

- Results root: `{results_root}`
- Dedicated Qwen3-30B v7 analysis tables: `{analysis_dir}`
- Exact final checkpoint: `{FINAL_BUDGET:,}` downstream tokens
- Seeds: {', '.join(str(seed) for seed in EXPECTED_SEEDS)}
- Publication validity: `test_fairness_ready == True`; an available diagnostic `coverage_valid` flag is also required
...
- High-performance threshold: `{high_accuracy_threshold:.2f}`
...
- `bias_in_bios_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`: Panel A is the development-only anytime proxy; Panel B shows the exact 5M holdout nR2 for seeds 72, 73, and 74 with method mean ± SD in the legend.
- Fairness-invalid rows are excluded from main publication figures and retained in `all_evaluations_raw.parquet` and the coverage diagnostic figure.

## Main figure files

- `bias_in_bios_qwen3_30b_v7_5m_final_nr2_development_and_holdout.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_stepwise_coverage_validity.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_stepwise_hv_exact_gap.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_exact_nr2_hv_gap.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_balanced_exact_metrics.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_attainment_macro_f1_cost.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_attainment_macro_f1_unfairness.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_pareto_cost_unfairness.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_test_pareto_3d.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_method_comparison_hv_nr2_gap.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_coverage_validity_diagnostics.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_high_macro_f1_operating_points.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare.*`
- `bias_in_bios_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if needs_rebuild:
        if args.no_auto_rebuild and not args.rebuild_analysis:
            raise RuntimeError(
                f"{analysis_dir} is missing, stale, or non-Qwen. "
    ...
    selected.to_csv(
        figure_dir / "bias_in_bios_qwen3_30b_v7_5m_final_selected_runs.csv", index=False
    )
    checkpoint_metrics.to_csv(
        figure_dir / "bias_in_bios_qwen3_30b_v7_5m_final_checkpoint_run_metrics.csv", index=False
    )
    final_evaluations.to_parquet(
        figure_dir / "bias_in_bios_qwen3_30b_v7_5m_final_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir / "bias_in_bios_qwen3_30b_v7_5m_final_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
        figure_dir / "bias_in_bios_qwen3_30b_v7_5m_final_stepwise_dev_nr2_proxy.csv", index=False
    )
    ...
    plot_empirical_attainment(
        ...
        filename="bias_in_bios_qwen3_30b_v7_5m_final_attainment_macro_f1_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="bias_in_bios_qwen3_30b_v7_5m_final_attainment_macro_f1_unfairness",
    )
    ...
    high_accuracy_points = plot_high_macro_f1_operating_points(
        final_evaluations,
        figure_dir,
    ...
    plot_trifair_diagnostic(
        ...
        filename="bias_in_bios_qwen3_30b_v7_5m_final_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="bias_in_bios_qwen3_30b_v7_5m_final_trifair_fewshot_unfairness",
    )
    ...
    for name in generated:
        print(f"  {name}")
    print(f"\nBias-in-Bios 5M figures written to: {figure_dir.resolve()}")
