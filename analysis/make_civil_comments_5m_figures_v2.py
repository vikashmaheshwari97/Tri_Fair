"""Create publication-style Civil Comments 5M v2 figures for Tri-Fair and NSGA-II-PO-Fair.

This script is specific to the upgraded Civil Comments study stored under
...
Key safeguards
--------------
1. Only the six intended Qwen-3-30B / Civil Comments / 5M runs are accepted:
...
3. Main multi-objective metrics require both ``dev_fairness_ready`` and
   ``test_fairness_ready``. Minimum-class-recall shortfalls are not silently
   discarded because Civil Comments v2 already folds them into the optimized
   unfairness objective through ``utility_penalty``.
4. Objective normalization is deterministic and matches the v2 Civil Comments objective
...
Default usage on Rocket::
    ...
    python -m analysis.make_civil_comments_5m_figures_v2 --rebuild-analysis --strict
...
class-recall and equalized-odds diagnostics, high-accuracy operating-point comparisons,
...
UNFAIRNESS_AXIS_LABEL = "Statistical Equalized-Odds Unfairness ↓"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/civil_comments_5m_v2",
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_civil_comments_v2.json",
    ...
    parser.add_argument(
        ...
        help="Fixed cost-objective upper bound used by the v2 Civil Comments configuration.",
    )
    parser.add_argument(
        "--overwrite-bounds",
        action="store_true",
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended Civil Comments/Qwen runs and remove archived copies."""
    out = frame.copy()
    if "run_dir" in out:
...
def attach_civil_diagnostics(frame: pd.DataFrame) -> pd.DataFrame:
    """Parse Civil Comments v2 diagnostics and attach audit-friendly columns.

    The native objective already includes the class-recall utility penalty, so
    low-recall candidates are retained rather than silently removed.  Main
    multi-objective metrics require statistically ready development and holdout
    estimates; utility validity is reported separately as a diagnostic.
    """
    out = frame.copy()
    numeric_keys = (
        "equalized_odds_rms",
        ...
        "utility_penalty",
        "equal_opportunity_rms",
        "false_positive_gap_rms",
        "demographic_parity_rms",
        "max_abs_tpr_gap",
        "max_abs_fpr_gap",
    ...
    for split in ("dev", "test"):
        ...
        required = out[f"{split}_required_minimum_class_recall"].fillna(0.50)
        minimum = out[f"{split}_minimum_class_recall"]
        penalty = out[f"{split}_utility_penalty"].fillna(0.0)
        out[f"{split}_utility_valid"] = minimum.ge(required)
...
def publication_valid_evaluations(
    ...
    *,
    ...
    Civil Comments v2 folds the minimum-class-recall shortfall directly into
    ``test_fairness``.  Consequently utility-penalized rows remain legitimate
    candidates; excluding them would alter the optimized objective after the
    fact.  Their recall and penalty diagnostics are preserved for audit plots.
    """
    enriched = attach_civil_diagnostics(frame)
    if strict:
        missing = [
            column
            for column in (
        ...
        if missing:
            raise ValueError(f"Evaluation table lacks Civil Comments v2 fields: {missing}")
    valid = enriched[enriched["publication_valid"]].copy()
    if valid.empty:
        raise RuntimeError(
            "No development/test-ready rows remain after requiring statistically "
            "ready development and holdout fairness estimates"
        )
...
def ensure_v2_bounds(
    ...
    *,
    ...
    """Create or validate deterministic Civil Comments v2 objective bounds."""
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="civil_comments_v2_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
    print(f"Wrote deterministic Civil Comments v2 bounds: {path.resolve()}")
    return store
...
def write_analysis_manifest(
    ...
    *,
    ...
    payload = {
        ...
        "utility_rule": (
            "minimum class recall below the configured threshold is retained "
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild exact v2 tables using readiness-valid Civil Comments rows."""
    ...
    print(f"\nRebuilding upgraded Civil Comments 5M tables from: {results_root.resolve()}")
    ...
    raw_evaluations = attach_civil_diagnostics(raw_evaluations)
...
def read_inputs(
    ...
    if missing:
        raise FileNotFoundError(
            "Missing required Civil Comments v2 analysis outputs:\n"
            + "\n".join(missing)
    ...
    raw_evaluations = attach_civil_diagnostics(
...
def stepwise_development_r2_proxy(
    ...
    *,
    ...
    for keys, group in step_results.groupby(group_cols, sort=True, dropna=False):
        ...
        finite_mask = np.isfinite(quality) & np.isfinite(cost) & np.isfinite(fairness)
        ready = (
            group["fairness_ready"].fillna(False).astype(bool).to_numpy()
        ...
        minimum_recall = np.full(len(group), np.nan, dtype=float)
        ...
        if "fairness_diagnostics_json" in group:
            ...
            minimum_recall = pd.to_numeric(
                diagnostics.map(lambda value: value.get("minimum_class_recall", np.nan)),
                errors="coerce",
            ).to_numpy(dtype=float)
            utility_penalty = pd.to_numeric(
                diagnostics.map(lambda value: value.get("utility_penalty", np.nan)),
                errors="coerce",
            ).to_numpy(dtype=float)

        ready_fraction = (
            float(np.mean(ready[finite_mask])) if finite_mask.any() else float("nan")
        )
        ...
        valid_recalls = minimum_recall[valid_mask]
        mean_minimum_recall = (
        ...
        rows.append(
            {
                **meta,
                ...
                "dev_mean_minimum_class_recall": mean_minimum_recall,
            }
        )
    if not rows:
...
def plot_stepwise_nr2_proxy(
    ...
    *,
    ...
    ax.set_title("Civil Comments v2 — Qwen-3-30B")
    ...
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_stepwise_dev_nr2_proxy")
    ...
    *,
    ...
    specifications = [
        ...
        (
            ...
            "Minimum-Recall-Valid Candidate Fraction",
            "Recall-Valid Fraction",
        ),
    ]
    for ax, (column, title, ylabel) in zip(axes, specifications):
        stats = step_band_statistics(step_proxy, column, max_budget=max_budget)
        for optimizer in OPTIMIZER_ORDER:
            if optimizer not in stats:
                continue
            grid, mean, lower, upper = stats[optimizer]
    ...
    fig.suptitle("Civil Comments v2 — Development Readiness and Utility")
    save_figure(
        fig,
        outdir,
        "civil_comments_qwen3_5m_v2_stepwise_readiness_utility",
    )
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("Civil Comments v2 — Qwen-3-30B")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments v2 — Qwen-3-30B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_exact_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    if handles:
        axes[0].legend(frameon=False, loc="best")
    fig.suptitle("Civil Comments v2 — Qwen-3-30B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Civil Comments v2 — Qwen-3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Civil Comments v2 — 5M Cost vs Test Unfairness")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Civil Comments v2 — Qwen-3-30B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Accuracy ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments v2 — Qwen-3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_v2_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Civil Comments v2 — Qwen-3-30B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Accuracy")
    ax.grid(True, alpha=0.25)
...
def utility_validity_summary(final_raw: pd.DataFrame) -> pd.DataFrame:
    data = attach_civil_diagnostics(final_raw)
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            group = data[
            ...
            total = int(len(group))
            ready = int(group["publication_valid"].sum()) if total else 0
            utility_valid = int(group["test_utility_valid"].sum()) if total else 0
            penalized = int(group["test_utility_penalized"].sum()) if total else 0
            rows.append(
                {
                    "optimizer": optimizer,
                    ...
                    "minimum_recall_valid_candidates": utility_valid,
                    "utility_penalized_candidates": penalized,
                    "ready_fraction": float(ready / total) if total else float("nan"),
                    "minimum_recall_valid_fraction": (
                        float(utility_valid / total) if total else float("nan")
                    ),
                    "mean_minimum_class_recall": pd.to_numeric(
                        group["test_minimum_class_recall"], errors="coerce"
                    ).mean(),
                    "mean_equalized_odds_rms": pd.to_numeric(
                        group["test_equalized_odds_rms"], errors="coerce"
                    ).mean(),
                    "mean_utility_penalty": pd.to_numeric(
                        group["test_utility_penalty"], errors="coerce"
                    ).mean(),
                    "mean_worst_group_accuracy": pd.to_numeric(
                        group["test_worst_group_accuracy"], errors="coerce"
                    ).mean(),
                }
            )
    return pd.DataFrame(rows)
...
def plot_fairness_validity_diagnostics(
    ...
    summary.to_csv(
        outdir / "civil_comments_qwen3_5m_v2_validity_by_seed.csv",
        index=False,
    )
    ...
    for index, optimizer in enumerate(OPTIMIZER_ORDER):
        ...
        axes[0, 0].bar(
            positions + offset,
            method["minimum_recall_valid_fraction"],
            width=width,
            color=COLORS[optimizer],
            alpha=0.82,
            label=DISPLAY_NAME[optimizer],
        )
    axes[0, 0].set_xticks(positions, [str(seed) for seed in EXPECTED_SEEDS])
    axes[0, 0].set_xlabel("Seed")
    axes[0, 0].set_ylabel("Candidate Fraction")
    axes[0, 0].set_ylim(0.0, 1.05)
    axes[0, 0].set_title("Minimum Class Recall ≥ 0.50")
    axes[0, 0].grid(True, axis="y", alpha=0.25)
    axes[0, 0].legend(frameon=False)

    penalized = data[data["test_utility_penalized"]]
    unpenalized = data[~data["test_utility_penalized"]]
    axes[0, 1].scatter(
        penalized["test_equalized_odds_rms"],
        penalized["test_minimum_class_recall"],
        marker="x",
        color="0.5",
        alpha=0.75,
        s=34,
    ...
    for optimizer in OPTIMIZER_ORDER:
        group = unpenalized[unpenalized["optimizer"] == optimizer]
        axes[0, 1].scatter(
            group["test_equalized_odds_rms"],
    ...
    axes[0, 1].axhline(0.50, color="0.5", linestyle="--", linewidth=1)
    axes[0, 1].set_xlabel("Equalized-Odds RMS Disparity ↓")
    axes[0, 1].set_ylabel("Minimum Class Recall ↑")
    axes[0, 1].set_title("Native Disparity and Utility Gate")
    ...
    for optimizer in OPTIMIZER_ORDER:
        ...
        axes[1, 0].scatter(
            group["test_average_accuracy"],
            group["test_worst_group_accuracy"],
            marker=MARKERS[optimizer],
            color=COLORS[optimizer],
            alpha=0.60,
            s=30,
    ...
    limits = [
        float(np.nanmin(pd.to_numeric(data["test_worst_group_accuracy"], errors="coerce"))),
        float(np.nanmax(pd.to_numeric(data["test_average_accuracy"], errors="coerce"))),
    ]
    axes[1, 0].plot(limits, limits, color="0.5", linestyle="--", linewidth=1)
    axes[1, 0].set_xlabel("Average Test Accuracy")
    axes[1, 0].set_ylabel("Worst-Group Accuracy ↑")
    axes[1, 0].set_title("Average vs Worst-Group Performance")
    axes[1, 0].grid(True, alpha=0.25)
    axes[1, 0].legend(frameon=False)

    components = [
        ("test_equal_opportunity_rms", "TPR gap RMS"),
        ("test_false_positive_gap_rms", "FPR gap RMS"),
        ("test_demographic_parity_rms", "DP gap RMS"),
    ]
    ...
    axes[1, 1].set_xticks(x, [label for _, label in components], rotation=8)
    axes[1, 1].set_ylabel("RMS Disparity ↓")
    axes[1, 1].set_title("Identity-Disparity Decomposition")
    axes[1, 1].grid(True, axis="y", alpha=0.25)
    axes[1, 1].legend(frameon=False)

    fig.suptitle("Civil Comments v2 — Qwen-3-30B at 5M")
    save_figure(
        fig,
        outdir,
        "civil_comments_qwen3_5m_v2_fairness_diagnostics",
    )
...
def high_accuracy_operating_points(
    ...
    *,
    ...
    data = attach_civil_diagnostics(final)
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            group = data[
            ...
            rows.append(
                {
                    ...
                    "minimum_class_recall": float(row["test_minimum_class_recall"]),
                    "equalized_odds_rms": float(row["test_equalized_odds_rms"]),
                    "utility_penalty": float(row["test_utility_penalty"]),
                    "worst_group_accuracy": float(row["test_worst_group_accuracy"]),
                    "average_minus_worst_gap": float(row["test_average_minus_worst_gap"]),
...
def plot_high_accuracy_operating_points(
    ...
    *,
    ...
    points.to_csv(
        outdir / "civil_comments_qwen3_5m_v2_high_accuracy_operating_points.csv",
        index=False,
    )
    ...
    specifications = [
        ...
        ("test_fairness", "Native Unfairness ↓", UNFAIRNESS_AXIS_LABEL),
        ("minimum_class_recall", "Minimum Class Recall ↑", "Minimum Recall"),
    ]
    for ax, (column, title, ylabel) in zip(axes.ravel(), specifications):
        for optimizer in OPTIMIZER_ORDER:
        ...
        ax.set_title(title)
        ax.grid(True, alpha=0.25)
    axes[0, 0].axhline(threshold, color="0.5", linestyle="--", linewidth=1)
    axes[1, 1].axhline(0.50, color="0.5", linestyle="--", linewidth=1)
    axes[0, 0].legend(frameon=False)
    fig.suptitle(
        f"Civil Comments v2 — Lowest Native Unfairness with Accuracy ≥ {threshold:.2f}"
    )
    save_figure(
        fig,
        outdir,
        "civil_comments_qwen3_5m_v2_high_accuracy_operating_points",
    )
    return points
...
def write_summary_tables(
    ...
    diagnostics = utility_validity_summary(final_raw_evaluations)
    best_points = attach_civil_diagnostics(best_quality_points(final_evaluations))
    rows: list[dict[str, object]] = []
    for optimizer in OPTIMIZER_ORDER:
        ...
        diagnostic_method = diagnostics[diagnostics["optimizer"] == optimizer]
        rows.append(
            {
                "Method": DISPLAY_NAME[optimizer],
                "Seeds": int(metrics["seed"].nunique()),
                "Raw 5M candidates": int(len(raw_eval)),
                "Dev/test-ready 5M candidates": int(len(valid_eval)),
                "Minimum-recall-valid fraction mean": diagnostic_method[
                    "minimum_recall_valid_fraction"
                ].mean(),
                ...
                "Best-quality mean accuracy ↑": best["test_quality"].mean(),
                "Best-quality mean cost ↓": best["test_cost"].mean(),
                "Best-quality mean native unfairness ↓": best[
                    "test_fairness"
                ].mean(),
                "Best-quality mean minimum recall ↑": best[
                    "test_minimum_class_recall"
                ].mean(),
                "Best-quality mean worst-group accuracy ↑": best[
                    "test_worst_group_accuracy"
                ].mean(),
                "High-accuracy mean accuracy ↑": high["test_quality"].mean(),
                "High-accuracy mean cost ↓": high["test_cost"].mean(),
                "High-accuracy mean native unfairness ↓": high[
                    "test_fairness"
                ].mean(),
                "High-accuracy mean minimum recall ↑": high[
                    "minimum_class_recall"
                ].mean(),
                "High-accuracy mean worst-group accuracy ↑": high[
                    "worst_group_accuracy"
                ].mean(),
            }
        )
    ...
    table.to_csv(
        outdir / "civil_comments_qwen3_5m_v2_summary_table.csv",
        index=False,
    )
    (outdir / "civil_comments_qwen3_5m_v2_summary_table.md").write_text(
        table.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    diagnostics.to_csv(
        outdir / "civil_comments_qwen3_5m_v2_validity_summary.csv",
        index=False,
    )
    (outdir / "civil_comments_qwen3_5m_v2_validity_summary.md").write_text(
        diagnostics.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    print("\nCivil Comments v2 5M method summary")
...
def write_readme(
    ...
    *,
    ...
    content = f"""# Civil Comments v2 / Qwen-3-30B / 5M publication figures

Generated by `python -m analysis.make_civil_comments_5m_figures_v2`.
...
- Low minimum-class-recall candidates are retained because their shortfall is already folded into the native unfairness objective through `utility_penalty`.
...
## Native fairness objective

Civil Comments v2 reports identity-wise equalized-odds disparity. TPR and FPR
present-versus-absent gaps are RMS aggregated across the eight overlapping
identity attributes. The optimized loss is the larger of the equalized-odds RMS
and the normalized minimum-class-recall shortfall below 0.50.
...
- `civil_comments_qwen3_5m_v2_stepwise_dev_nr2_proxy.*`: development-only anytime proxy, not exact holdout nR2.
- Raw and readiness-valid evaluation tables are both retained for auditability.

## Main figure files

- `civil_comments_qwen3_5m_v2_stepwise_dev_nr2_proxy.*`
- `civil_comments_qwen3_5m_v2_stepwise_readiness_utility.*`
- `civil_comments_qwen3_5m_v2_stepwise_hv_exact_gap.*`
- `civil_comments_qwen3_5m_v2_exact_nr2_hv_gap.*`
- `civil_comments_qwen3_5m_v2_balanced_exact_metrics.*`
- `civil_comments_qwen3_5m_v2_attainment_accuracy_cost.*`
- `civil_comments_qwen3_5m_v2_attainment_accuracy_unfairness.*`
- `civil_comments_qwen3_5m_v2_pareto_cost_unfairness.*`
- `civil_comments_qwen3_5m_v2_test_pareto_3d.*`
- `civil_comments_qwen3_5m_v2_method_comparison_hv_nr2_gap.*`
- `civil_comments_qwen3_5m_v2_fairness_diagnostics.*`
- `civil_comments_qwen3_5m_v2_high_accuracy_operating_points.*`
- `civil_comments_qwen3_5m_v2_trifair_fewshot_outputshare.*`
- `civil_comments_qwen3_5m_v2_trifair_fewshot_unfairness.*`
- `civil_comments_qwen3_5m_v2_trifair_fewshot_minimum_recall.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    selected.to_csv(
        figure_dir / "civil_comments_qwen3_5m_v2_selected_runs.csv", index=False
    )
    checkpoint_metrics.to_csv(
        figure_dir / "civil_comments_qwen3_5m_v2_checkpoint_run_metrics.csv", index=False
    )
    final_evaluations.to_parquet(
        figure_dir / "civil_comments_qwen3_5m_v2_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir / "civil_comments_qwen3_5m_v2_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
        figure_dir / "civil_comments_qwen3_5m_v2_stepwise_dev_nr2_proxy.csv", index=False
    )
    ...
    plot_empirical_attainment(
        ...
        filename="civil_comments_qwen3_5m_v2_attainment_accuracy_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_5m_v2_attainment_accuracy_unfairness",
    )
    ...
    plot_fairness_validity_diagnostics(final_raw_evaluations, figure_dir)
    ...
    plot_trifair_diagnostic(
        ...
        filename="civil_comments_qwen3_5m_v2_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_5m_v2_trifair_fewshot_unfairness",
    )
    plot_trifair_diagnostic(
        attach_civil_diagnostics(final_evaluations),
        figure_dir,
        color_col="test_minimum_class_recall",
        color_label="Minimum Class Recall ↑",
        filename="civil_comments_qwen3_5m_v2_trifair_fewshot_minimum_recall",
    )
    ...
    for name in generated:
        print(f"  {name}")
    print(f"\nCivil Comments v2 5M figures written to: {figure_dir.resolve()}")
