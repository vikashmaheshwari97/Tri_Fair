...
* quality-first, cost-first, and fairness-first trajectories;
* final nR2/HV/gap comparisons;
* final quality-first, cost-first, and fairness-first comparisons;
...
POLICY_SPECS = {
    "quality_first": (
        ("quality_first_test_quality", "Quality-First Test Quality ↑", "Test Quality"),
        ("quality_first_test_cost", "Quality-First Test Cost ↓", COST_SHORT_LABEL),
        ("quality_first_test_fairness", "Quality-First Test Unfairness ↓", UNFAIRNESS_AXIS_LABEL),
    ),
    "cost_first": (
        ("cost_first_test_quality", "Cost-First Test Quality ↑", "Test Quality"),
        ("cost_first_test_cost", "Cost-First Test Cost ↓", COST_SHORT_LABEL),
        ("cost_first_test_fairness", "Cost-First Test Unfairness ↓", UNFAIRNESS_AXIS_LABEL),
    ),
    "fairness_first": (
        ("fairness_first_test_quality", "Fairness-First Test Quality ↑", "Test Quality"),
        ("fairness_first_test_cost", "Fairness-First Test Cost ↓", COST_SHORT_LABEL),
        ("fairness_first_test_fairness", "Fairness-First Test Unfairness ↓", UNFAIRNESS_AXIS_LABEL),
    ),
}
SCORECARD_METRICS = (
    ("noisy_r2_3d", "nR2 ↓"),
    ...
    ("quality_first_test_quality", "Quality-First Quality ↑"),
    ("cost_first_test_cost", "Cost-First Cost ↓"),
    ("fairness_first_test_fairness", "Fairness-First Unfairness ↓"),
...
def build_locf_grid(
    ...
    *,
    ...
    metric_columns = [metric for metric, _, _ in TRAJECTORY_METRICS]
    for policy_specs in POLICY_SPECS.values():
        metric_columns.extend(metric for metric, _, _ in policy_specs)
...
def plot_policy_trajectories(
    ...
    *,
    ...
    policy_title = policy.replace("_", " ").title()
    fig, axes = plt.subplots(1, 3, figsize=(13.4, 4.1), constrained_layout=True)
    for metric_index, (ax, (metric, title, ylabel)) in enumerate(zip(axes, specs)):
        add_initial_point(ax, initial, metric, label=metric_index == 0)
        for optimizer in OPTIMIZER_ORDER:
            method_seed = seed_grid[seed_grid["optimizer"] == optimizer]
            for _, group in method_seed.groupby("seed", sort=True):
    ...
    fig.suptitle(
        f"{DATASET_DISPLAY[dataset]} — Qwen-3-30B v5 {policy_title} Holdout Trajectories"
    )
    stem = (
        f"{DATASET_SLUG[dataset]}_qwen3_5m_v5_"
        f"{policy}_holdout_trajectory"
    )
    save_figure(fig, outdir, stem, formats)
...
def plot_final_policy(
    ...
    *,
    ...
    fig.suptitle(
        f"{DATASET_DISPLAY[dataset]} — Qwen-3-30B v5 Final {policy_title} Point"
    )
    stem = f"{DATASET_SLUG[dataset]}_qwen3_5m_v5_final_{policy}_metrics"
    save_figure(fig, outdir, stem, formats)
...
def plot_initial_vs_final_policy(
    initial: pd.DataFrame,
    final: pd.DataFrame,
    dataset: str,
    outdir: Path,
    ...
    *,
    ...
    fig.suptitle(
        f"{DATASET_DISPLAY[dataset]} — Initial vs Final {policy_title} Points"
    )
    stem = (
        f"{DATASET_SLUG[dataset]}_qwen3_5m_v5_"
        f"initial_vs_final_{policy}"
    )
    save_figure(fig, outdir, stem, formats)
...
def select_posthoc_best_objectives(final: pd.DataFrame) -> pd.DataFrame:
    """Select independent holdout optima for descriptive diagnostics only.

    These rows are not a single jointly feasible operating point: best quality,
    lowest cost, and lowest unfairness may be three different prompts.
    """
    rows: list[dict[str, object]] = []
    rules = {
        "best_quality": (
    ...
    for optimizer in OPTIMIZER_ORDER:
        for seed in EXPECTED_SEEDS:
            ...
            for objective, (columns, ascending) in rules.items():
                ...
                rows.append(
                    {
                        ...
                        "seed": int(seed),
                        "objective": objective,
                        "test_quality": float(row["test_quality"]),
                        "test_cost": float(row["test_cost"]),
                        "test_fairness": float(row["test_fairness"]),
...
def plot_posthoc_best_objectives(
    final: pd.DataFrame,
    dataset: str,
    outdir: Path,
    formats: Sequence[str],
) -> pd.DataFrame:
    points = select_posthoc_best_objectives(final)
    points.to_csv(
        outdir
        / f"{DATASET_SLUG[dataset]}_qwen3_5m_v5_posthoc_best_objectives.csv",
        index=False,
    )
    if points.empty:
        return points
    ...
    specs = (
        ("best_quality", "test_quality", "Best Test Quality ↑", QUALITY_AXIS_LABEL),
        ("lowest_cost", "test_cost", "Lowest Test Cost ↓", COST_SHORT_LABEL),
        (
            "lowest_unfairness",
            "test_fairness",
            "Lowest Test Unfairness ↓",
            UNFAIRNESS_AXIS_LABEL,
        ),
    )
    for ax, (objective, column, title, ylabel) in zip(axes, specs):
        subset = points[points["objective"] == objective]
        for optimizer in OPTIMIZER_ORDER:
            group = subset[subset["optimizer"] == optimizer].sort_values("seed")
            ax.plot(
                group["seed"],
                group[column],
                color=COLORS[optimizer],
    ...
    axes[0].legend(frameon=False)
    fig.suptitle(
        f"{DATASET_DISPLAY[dataset]} — Independent Post-hoc Holdout Optima"
    )
    fig.text(
        0.5,
        0.005,
        "Descriptive oracle diagnostics only; each panel may select a different prompt.",
        ha="center",
        fontsize=8,
        alpha=0.75,
    )
    stem = f"{DATASET_SLUG[dataset]}_qwen3_5m_v5_posthoc_best_objectives"
    save_figure(fig, outdir, stem, formats)
    return points
...
def output_cost_share(frame: pd.DataFrame) -> np.ndarray:
    ...
    input_tokens = numeric(frame["test_input_tokens"])
    output_tokens = numeric(frame["test_output_tokens"])
...
def trifair_final_candidates(final: pd.DataFrame) -> pd.DataFrame:
    data = final[final["optimizer"] == "Tri-Fair-v5"].copy()
    if "is_incumbent" in data and data["is_incumbent"].notna().any():
        ...
        if not incumbent.empty:
            data = incumbent.copy()
    if data.empty:
        raise RuntimeError("No Tri-Fair-v5 final candidates were found")
    if "few_shots_json" in data:
        data["fewshot_count"] = data["few_shots_json"].apply(
            parse_fewshot_count
        )
    else:
...
def plot_trifair_fewshot_diagnostic(
    ...
    *,
    ...
    needed = {"test_input_tokens", "test_output_tokens"}
    if not needed.issubset(final.columns):
        warnings.warn(
            f"Skipping Tri-Fair few-shot diagnostic for {dataset}; "
            f"missing {sorted(needed - set(final.columns))}"
        )
        return
    data = trifair_final_candidates(final)
    values = pd.to_numeric(data[color_col], errors="coerce").to_numpy(dtype=float)
    ...
    ax.set_title(
        f"Tri-Fair v5 on {DATASET_DISPLAY[dataset]} — Qwen-3-30B at 5M"
    )
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel(QUALITY_AXIS_LABEL)
    ax.grid(True, alpha=0.25)
