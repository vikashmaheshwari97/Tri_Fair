"""Create publication-style Civil Comments 5M figures for Tri-Fair and NSGA-II-PO-Fair.
...
1. It never reads the generic ``analysis/output`` directory by default.  It uses
   a dedicated ``analysis/output/civil_comments_5m`` directory, so stale 1M CSV/Parquet
...
Default one-command usage on Rocket::
    ...
    python -m analysis.make_civil_comments_5m_figures
...
DEFAULT_BUDGETS = (1_000_000, 5_000_000)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/civil_comments_5m",
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended Civil Comments/Qwen runs and remove archived pilot copies."""
    out = frame.copy()
    if "run_dir" in out:
...
def rebuild_analysis_outputs(
    *,
    ...
    """Rebuild Civil Comments-only 5M tables using the repository's main analysis code."""
    if not results_root.is_dir():
        raise FileNotFoundError(
            f"Results root does not exist: {results_root}\n"
            "Extract/copy the Civil Comments 5M bundle under results/tri_fair, or pass "
    ...
    print(f"\nRebuilding Civil Comments 5M analysis tables from: {results_root.resolve()}")
...
def plot_stepwise_nr2_proxy(step_proxy: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Civil Comments — Qwen-3-30B")
    ...
    save_figure(fig, outdir, "civil_comments_qwen3_5m_stepwise_dev_nr2_proxy")
...
def plot_stepwise_hv_and_gap(
    ...
    fig.suptitle("Civil Comments — Qwen-3-30B")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_stepwise_hv_checkpoint_gap")
...
def plot_checkpoint_metric_on_axis(
    ...
    *,
    ...
    if observed:
        positions = [value / 1_000_000.0 for value in observed]
        labels = [
            f"{value / 1_000_000:g}M" if value >= 1_000_000 else f"{value / 1_000:g}K"
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments — Qwen-3-30B: Exact Checkpoint MO Metrics")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_checkpoint_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    axes[0].legend(frameon=False, loc="best")
    fig.suptitle("Civil Comments — Qwen-3-30B: Budget Trajectory")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_balanced_budget_trajectory")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("Civil Comments — Qwen-3-30B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_5m_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("Civil Comments — Qwen-3-30B 5M Test Pareto Fronts")
    ax.set_xlabel("Cost ↓")
    ax.set_ylabel("Unfairness ↓")
    ax.set_zlabel("Accuracy ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "civil_comments_qwen3_5m_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("Civil Comments — Qwen-3-30B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "civil_comments_qwen3_5m_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on Civil Comments — Qwen-3-30B at 5M")
    ax.set_xlabel("Avg. Cost [$] per 1M Calls")
    ax.set_ylabel("Test Accuracy")
    ax.grid(True, alpha=0.25)
...
def write_summary_tables(
    ...
    for optimizer in OPTIMIZER_ORDER:
        ...
        best_per_seed = evaluations.copy()
        for column in ("test_quality", "test_cost", "test_fairness"):
            best_per_seed[column] = pd.to_numeric(
                best_per_seed[column], errors="coerce"
            )
        best_per_seed = (
            best_per_seed.dropna(
                subset=["seed", "test_quality", "test_cost", "test_fairness"]
            )
            .sort_values(
                ["seed", "test_quality", "test_cost", "test_fairness"],
                ascending=[True, False, True, True],
                kind="mergesort",
            )
            .groupby("seed", as_index=False, sort=True)
            .head(1)
            .reset_index(drop=True)
        )
        rows.append(
    ...
    table.to_csv(outdir / "civil_comments_qwen3_5m_summary_table.csv", index=False)
    (outdir / "civil_comments_qwen3_5m_summary_table.md").write_text(
...
def write_readme(outdir: Path, analysis_dir: Path, results_root: Path) -> None:
    content = f"""# Civil Comments / Qwen-3-30B 5M curated figures

Generated by `python -m analysis.make_civil_comments_5m_figures`.
...
- `civil_comments_qwen3_5m_stepwise_dev_nr2_proxy.*`: development-only stepwise proxy, not exact holdout nR2.

## Main figure files

- `civil_comments_qwen3_5m_stepwise_dev_nr2_proxy.*`
- `civil_comments_qwen3_5m_stepwise_hv_checkpoint_gap.*`
- `civil_comments_qwen3_5m_checkpoint_nr2_hv_gap.*`
- `civil_comments_qwen3_5m_balanced_budget_trajectory.*`
- `civil_comments_qwen3_5m_attainment_accuracy_cost.*`
- `civil_comments_qwen3_5m_attainment_accuracy_unfairness.*`
- `civil_comments_qwen3_5m_pareto_cost_unfairness.*`
- `civil_comments_qwen3_5m_test_pareto_3d.*`
- `civil_comments_qwen3_5m_method_comparison_hv_nr2_gap.*`
- `civil_comments_qwen3_5m_trifair_fewshot_outputshare.*`
- `civil_comments_qwen3_5m_trifair_fewshot_unfairness.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    selected.to_csv(figure_dir / "civil_comments_qwen3_5m_selected_runs.csv", index=False)
    checkpoint_metrics.to_csv(
        figure_dir / "civil_comments_qwen3_5m_checkpoint_run_metrics.csv", index=False
    )
    step_proxy.to_csv(
        figure_dir / "civil_comments_qwen3_5m_stepwise_dev_nr2_proxy.csv", index=False
    )
    ...
    plot_empirical_attainment(
        ...
        filename="civil_comments_qwen3_5m_attainment_accuracy_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_5m_attainment_accuracy_unfairness",
    )
    ...
    plot_trifair_diagnostic(
        ...
        filename="civil_comments_qwen3_5m_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="civil_comments_qwen3_5m_trifair_fewshot_unfairness",
    )
