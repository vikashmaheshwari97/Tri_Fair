"""Create publication-style BBQ 5M figures for GPT-OSS-120B.

This script is specific to the strict-budget GPT-OSS-120B BBQ study stored under
``results/tri_fair_v2_3_gptoss_final5m_fresh``. It isolates that final namespace
and applies the BBQ publication-validity rules before computing metrics.

Key safeguards
--------------
1. Only the six intended GPT-OSS-120B / BBQ / fresh-5M runs are accepted:
...
4. Objective normalization is deterministic and matches the BBQ objective
...
Default usage on Rocket::
    ...
    python -m analysis.make_bbq_5m_figures_gptoss --rebuild-analysis --strict
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-dir",
        default="analysis/output/bbq_gptoss120b_final5m",
        help="Dedicated GPT-OSS-120B BBQ analysis-table directory.",
    )
    parser.add_argument(
        "--figure-dir",
        default=None,
    ...
    parser.add_argument(
        "--bounds-file",
        default="analysis/normalization_bounds_bbq_gptoss120b_final5m.json",
        help="Dedicated deterministic GPT-OSS BBQ bounds file; Qwen bounds are rejected.",
    )
    parser.add_argument(
        "--cost-upper-bound",
        type=float,
...
def clean_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep only the intended BBQ/GPT-OSS runs and remove archived copies."""
    out = frame.copy()
    if "run_dir" in out:
    ...
    store = BoundsStore(
        {
            (DATASET, MODEL): Bounds(
                ...
                source="bbq_gptoss_fixed_objective_bounds",
            )
        }
    )
    store.save(path)
...
def rebuild_analysis_outputs(
    *,
    ...
    print(f"\nRebuilding GPT-OSS BBQ 5M tables from: {results_root.resolve()}")
...
def plot_stepwise_coverage_validity(
    ...
    *,
    ...
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_stepwise_coverage_validity")
...
def plot_stepwise_hv_and_gap(
    ...
    *,
    ...
    fig.suptitle("BBQ — GPT-OSS-120B")
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_stepwise_hv_exact_gap")
...
def plot_exact_checkpoint_mo_metrics(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("BBQ — GPT-OSS-120B: Exact Complete Checkpoints")
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_exact_nr2_hv_gap")
...
def plot_balanced_budget_trajectory(checkpoint_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    axes[0].legend(frameon=False, loc="best")
    fig.suptitle("BBQ — GPT-OSS-120B: Exact Balanced Operating Point")
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_balanced_exact_metrics")
...
def plot_empirical_attainment(
    ...
    *,
    ...
    ax.set_title("BBQ — GPT-OSS-120B at 5M")
...
def plot_cost_unfairness_projection(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_pareto_cost_unfairness")
...
def plot_three_objective_pareto(final: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_title("BBQ — GPT-OSS-120B 5M Test Pareto Fronts")
    ax.set_xlabel(COST_SHORT_LABEL)
    ax.set_ylabel(UNFAIRNESS_AXIS_LABEL)
    ax.set_zlabel("Accuracy ↑")
    ax.legend(frameon=False)
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_test_pareto_3d")
...
def plot_method_comparison_bars(final_metrics: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.suptitle("BBQ — GPT-OSS-120B at 5M (mean ± SD, 3 seeds)")
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_method_comparison_hv_nr2_gap")
...
def plot_trifair_diagnostic(
    ...
    *,
    ...
    ax.set_title("Tri-Fair on BBQ — GPT-OSS-120B at 5M")
    ax.set_xlabel(COST_AXIS_LABEL)
    ax.set_ylabel("Test Accuracy")
    ax.grid(True, alpha=0.25)
...
def plot_coverage_validity_diagnostics(final_raw: pd.DataFrame, outdir: Path) -> None:
    ...
    summary.to_csv(
        outdir / "bbq_gptoss120b_5m_final_coverage_validity_by_seed.csv", index=False
    )
    ...
    fig.suptitle("BBQ — GPT-OSS-120B at 5M")
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_coverage_validity_diagnostics")
...
def plot_high_accuracy_operating_points(
    ...
    *,
    ...
    points.to_csv(
        outdir / "bbq_gptoss120b_5m_final_high_accuracy_operating_points.csv",
        index=False,
    )
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 3.9), constrained_layout=True)
    specifications = [
    ...
    save_figure(fig, outdir, "bbq_gptoss120b_5m_final_high_accuracy_operating_points")
    return points
...
def write_summary_tables(
    ...
    table.to_csv(outdir / "bbq_gptoss120b_5m_final_summary_table.csv", index=False)
    (outdir / "bbq_gptoss120b_5m_final_summary_table.md").write_text(
        table.to_markdown(index=False, floatfmt=".4f") + "\n",
        encoding="utf-8",
    )
    coverage.to_csv(
        outdir / "bbq_gptoss120b_5m_final_coverage_summary.csv", index=False
    )
    (outdir / "bbq_gptoss120b_5m_final_coverage_summary.md").write_text(
...
def write_readme(
    ...
    *,
    ...
    content = f"""# BBQ / GPT-OSS-120B / 5M publication figures

Generated by `python -m analysis.make_bbq_5m_figures_gptoss`.
...
- `bbq_gptoss120b_5m_final_balanced_exact_metrics.*`
- `bbq_gptoss120b_5m_final_attainment_accuracy_cost.*`
- `bbq_gptoss120b_5m_final_attainment_accuracy_unfairness.*`
- `bbq_gptoss120b_5m_final_pareto_cost_unfairness.*`
- `bbq_gptoss120b_5m_final_test_pareto_3d.*`
- `bbq_gptoss120b_5m_final_method_comparison_hv_nr2_gap.*`
- `bbq_gptoss120b_5m_final_coverage_validity_diagnostics.*`
- `bbq_gptoss120b_5m_final_high_accuracy_operating_points.*`
- `bbq_gptoss120b_5m_final_trifair_fewshot_outputshare.*`
- `bbq_gptoss120b_5m_final_trifair_fewshot_unfairness.*`
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    selected.to_csv(
        figure_dir / "bbq_gptoss120b_5m_final_selected_runs.csv", index=False
    )
    checkpoint_metrics.to_csv(
        figure_dir / "bbq_gptoss120b_5m_final_checkpoint_run_metrics.csv", index=False
    )
    final_evaluations.to_parquet(
        figure_dir / "bbq_gptoss120b_5m_final_publication_valid_evaluations.parquet",
        index=False,
    )
    final_raw_evaluations.to_parquet(
        figure_dir / "bbq_gptoss120b_5m_final_raw_final_evaluations.parquet",
        index=False,
    )
    step_proxy.to_csv(
    ...
    plot_empirical_attainment(
        ...
        filename="bbq_gptoss120b_5m_final_attainment_accuracy_cost",
    )
    plot_empirical_attainment(
        final_evaluations,
        figure_dir,
        ...
        filename="bbq_gptoss120b_5m_final_attainment_accuracy_unfairness",
    )
    ...
    plot_trifair_diagnostic(
        ...
        filename="bbq_gptoss120b_5m_final_trifair_fewshot_outputshare",
    )
    plot_trifair_diagnostic(
        final_evaluations,
        figure_dir,
        ...
        filename="bbq_gptoss120b_5m_final_trifair_fewshot_unfairness",
    )
